...
C
     ...
     1    MEDJVCS(8),MEDJKCS(16,8),DX,TJV,TJKCS(16),
     1    TV(10),TK(10,16),VSEGV(8,4),VSEGK(8,4,16),
...
C       VRV,VRK-  VESSEL FLUXES SUMMED FROM THE VSEGV AND VSEGK
C
C VARIABLES LOCAL TO GEN.F
        DOUBLE PRECISION
...
C
     ...
     1       RFVCS(IVR,ISEG,IX+1,1))
        DO 134 ISOL=1,SOLS
  134   VSEGK(IVR,ISEG,ISOL)=VSEGK(IVR,ISEG,ISOL)+
     1    0.5*DX*(RDNUM(IVR)*RSC(IVR,ISEG,IX,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX,1))+
     1    0.5*DX*(RDNUM(IVR)*RSC(IVR,ISEG,IX+1,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX+1,1))
...
C
     ...
     1       RFVCS(IVR,ISEG,IX+1,1))
        DO 154 ISOL=1,SOLS
  154   VSEGK(IVR,ISEG,ISOL)=VSEGK(IVR,ISEG,ISOL)+
     1    0.5*DX*(RANUM(IVR)*RSC(IVR,ISEG,IX,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX,1))+
     1    0.5*DX*(RANUM(IVR)*RSC(IVR,ISEG,IX+1,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX+1,1))
...
C
     ...
     1       RFVCS(IVR,ISEG,IX+1,1))
        DO 164 ISOL=1,SOLS
  164   VSEGK(IVR,ISEG,ISOL)=VSEGK(IVR,ISEG,ISOL)+
     1    0.5*DX*(RANUM(IVR)*RSC(IVR,ISEG,IX,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX,1))+
     1    0.5*DX*(RANUM(IVR)*RSC(IVR,ISEG,IX+1,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX+1,1))
...
C
     ...
     1       RFVCS(IVR,ISEG,IX+1,1))
        DO 234 ISOL=1,SOLS
  234   VSEGK(IVR,ISEG,ISOL)=VSEGK(IVR,ISEG,ISOL)+
     1    0.5*DX*(RDNUM(IVR)*RSC(IVR,ISEG,IX,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX,1))+
     1    0.5*DX*(RDNUM(IVR)*RSC(IVR,ISEG,IX+1,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX+1,1))
...
C
     ...
     1       RFVCS(IVR,ISEG,IX+1,1))
        DO 264 ISOL=1,SOLS
  264   VSEGK(IVR,ISEG,ISOL)=VSEGK(IVR,ISEG,ISOL)+
     1    0.5*DX*(RANUM(IVR)*RSC(IVR,ISEG,IX,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX,1))+
     1    0.5*DX*(RANUM(IVR)*RSC(IVR,ISEG,IX+1,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX+1,1))
...
C
     ...
     1       RFVCS(IVR,ISEG,IX+1,1))
        DO 324 ISOL=1,SOLS
  324   VSEGK(IVR,ISEG,ISOL)=VSEGK(IVR,ISEG,ISOL)+
     1    0.5*DX*(RDNUM(IVR)*RSC(IVR,ISEG,IX,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX,1))+
     1    0.5*DX*(RDNUM(IVR)*RSC(IVR,ISEG,IX+1,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX+1,1))
...
C
     ...
     1       RFVCS(IVR,ISEG,IX+1,1))
        DO 344 ISOL=1,SOLS
  344   VSEGK(IVR,ISEG,ISOL)=VSEGK(IVR,ISEG,ISOL)+
     1    0.5*DX*(RANUM(IVR)*RSC(IVR,ISEG,IX,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX,1))+
     1    0.5*DX*(RANUM(IVR)*RSC(IVR,ISEG,IX+1,1)*
     1       RFKCS(IVR,ISEG,ISOL,IX+1,1))
...
C
        DO 480 K=1,RVCHOP(8,1)
        MEDJVCS(8)=MEDJVCS(8)+0.5*(MRJVCS(K)+MRJVCS(K+1))
        DO 479 J=1,SOLS
  479   MEDJKCS(J,8)=MEDJKCS(J,8)+0.5*(MRJKCS(J,K)+MRJKCS(J,K+1))
  480   CONTINUE
        DX=RCL(8,1)/FLOAT(RVCHOP(8,1))
        MEDJVCS(8)=MEDJVCS(8)*DX
        DO 481 J=1,SOLS
  481   MEDJKCS(J,8)=MEDJKCS(J,8)*DX
C
...
C
	WRITE (294,522) (SOL(I),I=1,4),(SOL(I),I=7,9),
     1   SOL(11),SOL(13),SOL(15)
  522   FORMAT(//,4X,'SLICE',8X,'VOL',7X,10(A5,7X))
        DIST=0.D0
        WRITE (294,524) DIST,MEDJVCS(8),
     1   (MEDJKCS(J,8),J=1,4),(MEDJKCS(J,8),J=7,9),
     1   MEDJKCS(11,8),MEDJKCS(13,8),MEDJKCS(15,8)
        DO 520 K=1,7
        IF (K.EQ.1) THEN
...
C
        SLV(1)=VSEGV(1,1)+VSEGV(1,4)+
     1    VSEGV(2,1)+VSEGV(2,4)+
     1    VSEGV(3,1)+VSEGV(3,4)+
     1    VSEGV(4,1)+VSEGV(4,4)+
     1    VSEGV(5,1)+VSEGV(5,4)+
     1    VSEGV(6,1)+VSEGV(6,4)+
     1    VSEGV(7,1)+VSEGV(7,4)
        SLV(2)=VSEGV(3,2)+VSEGV(3,3)+
     1    VSEGV(4,2)+VSEGV(4,3)+
     1    VSEGV(5,2)+VSEGV(5,3)+
     1    VSEGV(6,2)+VSEGV(6,3)+
     1    VSEGV(7,2)+VSEGV(7,3)
        DO 740 J=1,SOLS
        SLK(1,J)=VSEGK(1,1,J)+VSEGK(1,4,J)+
     1    VSEGK(2,1,J)+VSEGK(2,4,J)+
     1    VSEGK(3,1,J)+VSEGK(3,4,J)+
     1    VSEGK(4,1,J)+VSEGK(4,4,J)+
     1    VSEGK(5,1,J)+VSEGK(5,4,J)+
     1    VSEGK(6,1,J)+VSEGK(6,4,J)+
     1    VSEGK(7,1,J)+VSEGK(7,4,J)
        SLK(2,J)=VSEGK(3,2,J)+VSEGK(3,3,J)+
     1    VSEGK(4,2,J)+VSEGK(4,3,J)+
     1    VSEGK(5,2,J)+VSEGK(5,3,J)+
     1    VSEGK(6,2,J)+VSEGK(6,3,J)+
     1    VSEGK(7,2,J)+VSEGK(7,3,J)
  740   CONTINUE
C
        WRITE (294,741)
  741   FORMAT (//)
...
C
  ...
  706   TK(IVR,J)=RANUM(IVR)*RFKC(IVR,ISEG,J,1,1)-
     1    RANUM(IVR)*RFKC(IVR,ISEG,J,IEND,1)
        ENDIF
C
        WRITE (294,710) IVR, ISEG
        WRITE (294,712) VSEGV(IVR,ISEG),
     1    (VSEGK(IVR,ISEG,J),J=1,4),(VSEGK(IVR,ISEG,J),J=7,9),
     1    VSEGK(IVR,ISEG,11),VSEGK(IVR,ISEG,13),VSEGK(IVR,ISEG,15)
        WRITE (294,714) TV(IVR),
     1   (TK(IVR,J),J=1,4), (TK(IVR,J),J=7,9), 
     1   TK(IVR,11),TK(IVR,13),TK(IVR,15)
  710   FORMAT (/,2I5)
...
C
        VRV(1)=VSEGV(1,1)+VSEGV(1,4)
        VRV(2)=VSEGV(2,1)+VSEGV(2,4) 
        DO 732 K=3,7
  732   VRV(K)=VSEGV(K,1)+VSEGV(K,2)+VSEGV(K,3)+VSEGV(K,4) 
        DO 735 J=1,SOLS
        VRK(1,J)=VSEGK(1,1,J)+VSEGK(1,4,J)
        VRK(2,J)=VSEGK(2,1,J)+VSEGK(2,4,J) 
        DO 734 K=3,7
  734   VRK(K,J)=VSEGK(K,1,J)+VSEGK(K,2,J)+VSEGK(K,3,J)+VSEGK(K,4,J) 
  735   CONTINUE
C
	WRITE (294,642) (SOL(I),I=1,4),(SOL(I),I=7,9),
     1   SOL(11),SOL(13),SOL(15)
        DO 737 IVR=1,7
  737   WRITE (294,647) IVR,VRV(IVR),
