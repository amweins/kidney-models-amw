...
C
C
	DOUBLE PRECISION RDNUM,CURC(901),OSMC(901),OSMS(901),
...
C
	DO 240 IX = 1,CHOP+1
        FVC(IX,T) = FVC(IX,T)*RDNUM
        FVCS(IX,T) = FVCS(IX,T)*RDNUM
        FBC(IX,T) = FBC(IX,T)*RDNUM
	DO 238 ISOL = 1,SOLS
        FKC(ISOL,IX,T) = FKC(ISOL,IX,T)*RDNUM
        FKCS(ISOL,IX,T) = FKCS(ISOL,IX,T)*RDNUM
  238   CONTINUE
...
C
	WRITE (291,490) (SOL(I),I=1,4),(SOL(I),I=7,9),
...
C
  ...
  430   FORMAT(//,5X,'DIST',7X,'VC',5X,
	...
	DO 440 KX=1,CHOP+1
	DIST = DIST + CL/FLOAT(CHOP)
  440   WRITE (291,450) DIST,VC(KX,T),
...
C
C
	WRITE (291,460) (SOL(I),I=1,4),(SOL(I),I=7,9),
     1   SOL(11),SOL(13),SOL(15)
  460   FORMAT(1H1,//,5X,'DIST',6X,'FVC',4X,'FKC:',
	...
	DO 470 KX=1,CHOP+1
	DIST = DIST + CL/FLOAT(CHOP)
  470   WRITE (291,480) DIST, 1.D3*FVC(KX,T),
...
C
	WRITE (291,464) (SOL(I),I=1,4),(SOL(I),I=7,9),
     1   SOL(11),SOL(13),SOL(15)
...
C
	WRITE (291,462) (SOL(I),I=1,4),(SOL(I),I=7,9),
     1   SOL(11),SOL(13),SOL(15)
  462   FORMAT(//,5X,'DIST',6X,'FVCS',2X,'FKCS:',
	...
	DO 472 KX=1,CHOP+1
	DIST = DIST + CL/FLOAT(CHOP)
  472   WRITE (291,482) DIST,1.D3*FVCS(KX,T),
...
C
	...
	DO 710 KX=1,CHOP+1
	DIST = DIST + CL/FLOAT(CHOP)
  710   WRITE (292,715) DIST,1.D6*FVC(KX,T),
     1   PS(KX,T)-PC(KX,T)+IMPC(KX,T),
     1   1.D3*(OSMC(KX)-OSMS(KX)),1.D6*FVCS(KX,1),
     1   1.D6*RDNUM*FVCSE(KX,1), 1.D6*RDNUM*FVCSI(KX,1)
  715   FORMAT(8D16.8)
	RETURN
	END
