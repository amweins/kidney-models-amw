...
C
	FVM(X,T)=GAMMA((2+SOLS)+1)*1.D-6*MUM
	IMPM(X,T)=GAMMA((2+SOLS)+2)
	RETURN
C
C
  200   GAMMA(1)=VM(X,T)
...
C
	GAMMA((2+SOLS)+1)=FVM(X,T)*1.D+6/MUM
	GAMMA((2+SOLS)+2)=IMPM(X,T)
	RETURN
C
	END
