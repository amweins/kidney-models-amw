...
C
  200   CONTINUE
        ...
        RDPAGE(38,IX)=RFVM(INEPH,ISEG,IX)
...
C
	...
	ISEG=0
	WRITE (JUNIT,965) ISEG, SCAL(3)*WRDL(1,1,KK),1
	WRITE (JUNIT,965) ISEG, SCAL(3)*WRDL(7,1,KK),2
	DO 962 ISEG=1,3
        WRITE (JUNIT,965) ISEG, -SCAL(3)*WRRES(1,ISEG,KK),1
  962   WRITE (JUNIT,965) ISEG, -SCAL(3)*WRRES(7,ISEG,KK),2
        WRITE (JUNIT,965) 4, -SCAL(3)*WRRES(7,4,KK),2
        WRITE (JUNIT,965) 5, -SCAL(3)*WRRES(7,5,KK),2
	DO 964 ISEG=6,8
        WRITE (JUNIT,965) ISEG, -SCAL(3)*WRRES(1,ISEG,KK),1
  964   WRITE (JUNIT,965) ISEG, -SCAL(3)*WRRES(7,ISEG,KK),2
C
        WRITE (JUNIT,965) 9, SCAL(3)*WREX(1,8,KK),1
        WRITE (JUNIT,965) 9, SCAL(3)*WREX(7,8,KK),2
C
	NBARS=6
	WRITE (JUNIT,965) NBARS
	ISEG=0
	WRITE (JUNIT,965) ISEG, SCAL(3)*WRDL(1,9,KK),3
	DO 968 ISEG=9,12
  968   WRITE (JUNIT,965) ISEG, -SCAL(3)*WRRES(1,ISEG,KK),3
        ISEG=13
	WRITE (JUNIT,965) ISEG, SCAL(3)*WREX(1,12,KK),3
...
C
   ...
   20   FORMAT(' TYPE THE NAME OF VARIABLE NUMBER-',I2)
	READ 30, VNAME1(KWR)
   30   FORMAT(A5)
	IF(VNAME1(KWR).EQ.'cm') THEN
        SCAL(KWR) = 1.D3
        GO TO 40
        ELSE IF(VNAME1(KWR).EQ.'xm') THEN
        SCAL(KWR) = 1.D0
        GO TO 40
        ELSE IF(VNAME1(KWR).EQ.'fkm') THEN
        SCAL(KWR) = 6.D1
        GO TO 40
        ELSE
        SCAL(KWR) = 1.D0
...
C
  ...
  330   WRITE(IUNIT,399) (SCAL(KWR)*WRPAGE(KWR,IX),KWR=1,NWR+1)
  340   CONTINUE
C
C        DO 360 INEPH=3,6,3
        INEPH=6
        IADD=1
        DO 360 ISEG=1,8
        CHOP=RCHOP(INEPH,ISEG)
        ...
        RDPAGE(21,IX)=RLCHM(INEPH,ISEG,IX)
        RDPAGE(37,IX)=ROSMM(INEPH,ISEG,IX)
        RDPAGE(38,IX)=RFVM(INEPH+IADD,ISEG,IX)
        RDPAGE(54,IX)=RFKM(INEPH+IADD,ISEG,16,IX)
        RDPAGE(55,IX)=RFKM(INEPH+IADD,ISEG,17,IX)
        RDPAGE(56,IX)=RFKM(INEPH+IADD,ISEG,18,IX)
        RDPAGE(57,IX)=RNAE(INEPH,ISEG,IX)
        DO 342 ISOL=1,15
        RDPAGE(3+ISOL,IX)=RCM(INEPH,ISEG,ISOL,IX)
        RDPAGE(21+ISOL,IX)=RXM(INEPH,ISEG,ISOL,IX)
        RDPAGE(38+ISOL,IX)=RFKM(INEPH+IADD,ISEG,ISOL,IX)
  342   CONTINUE
  344   CONTINUE
        DO 348 IX=1,CHOP+1
        DO 346 KWR=1,NWR+1
