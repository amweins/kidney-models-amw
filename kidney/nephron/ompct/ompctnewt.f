	SUBROUTINE OMPCTNEWT(COUNT,SWOUT)
C
	INTEGER SOLS,TAU,T,TLIM,COUNT,EXP,NPAR,X,CHOP,SWOUT
C
...
C
	...
	IF (TAU.EQ.1) CALL OMPCTPTIM(NPAR)
C
...
C Abolish the interstitial step up at x=1 and x=chop+1
   ...
   25   CS(ISOL,CHOP+1,1)=CS(ISOL,CHOP,1)
	ELSE
	NN=4*(2+SOLS)+2
	ENDIF
C
...
C IF THE SPATIAL ITERATION FAILS TO CONVERGE THE PROGRAM STOPS.
	PRINT 980
  980   FORMAT (' TOO MANY ITERATIONS IN OMPCT')
	WRITE (19,982) COUNT
  982   FORMAT ('OMPCT ERROR IN EXP. ', I3)
	GO TO  9600
C
C THE SPATIAL STEP CONVERGED,AND
C THE PROGRAM PICKS UP AT THIS POINT:
C
  558   CALL OMPCTRESET(1)
  400   CONTINUE
C
	X=1
	CALL OMPCTGAM(2,GAMMA)
