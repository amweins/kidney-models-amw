...
C
C
C PUMP FLUXES INCLUDE Na on Na,K-ATPase in all 3 cell types,
C  luminal H,K-ATPase in Alpha and Beta cells, and
C  luminal H+-ATPase in alpha cells, plus peitubular H+-ATPase in beta cells.
C
	QICO2(1) = BCO2(1)*(AIE(1)*ATIS(1,1,X) + AI0(1)*ATIS(1,1,X))
C
	QICO2(2) = BCO2(1)*(AIE(2)*ATIS(2,1,X) + AI0(2)*ATIS(2,1,X))
     1   +BCO2(3)*AI0(2)*ATMI(2,2,X)
     1   -BCO2(2)*AI0(2)*(ATMI(2,12,X)+ATMI(2,2,X))
C
...
C
	...
	DO 230 K=1,2
	KK=K*(2+SOLS)
	PHI(2+4+KK)=PHI(2+4+KK)+PHI(2+5+KK)+PHI(2+6+KK)-1.D6*QICO2(K)
...
C   MASS CONSERVATION IN THE TIME-DEPENDENT CASE
  ...
  260   CONTINUE
	DO 262 K=1,2
	PHI(2+KK)=PHI(2+KK)+
     1   (SM(X,T)*FIVM(K,X,T)+SM(X,T-1)*FIVM(K,X,T-1))
	DO 262 I=1,SOLS
	PHI(2+I+KK)=PHI(2+I+KK)+
     1   (SM(X,T)*FIKM(K,I,X,T)+SM(X,T-1)*FIKM(K,I,X,T-1))
  262   CONTINUE
C
C
...
C   MASS CONSERVATION IN THE STEADY-STATE CASE
  ...
  280   CONTINUE
	DO 282 K=1,2
	PHI(2+KK)=PHI(2+KK)+ SM(X,T)*FIVM(K,X,T)
	DO 282 I=1,SOLS
	PHI(2+I+KK)=PHI(2+I+KK)+ SM(X,T)*FIKM(K,I,X,T)
  282   CONTINUE
C
C   AND FOR THE IMPERMEANT
C
...
C
	...
	PHI(2+6+KK)=PHI(2+6+KK)+
     1   1.D6*AM(X,T)*(KHY(5)*CM(6,X,T)-KDHY(5)*CM(5,X,T))+
     1   1.D6*AM(X,T-1)*(KHY(5)*CM(6,X,T-1)-KDHY(5)*CM(5,X,T-1))
	ELSE
	PHI(2+6+KK)=PHI(2+6+KK)+
     1   1.D6*AM(X,T)*(KHY(5)*CM(6,X,T)-KDHY(5)*CM(5,X,T))
	ENDIF
