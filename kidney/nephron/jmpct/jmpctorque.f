...
C
	DO 180 K=1,1
	LHP(K)=LHP0(K)*(1.0 + 
     1   0.5*DTANH(SCALMI*(TQM(X,T)/TQM0 -1.0)))
	NP(K)=NP0(K)*(1.0 + 
     1   0.5*DTANH(SCALIS*(TQM(X,T)/TQM0 -1.0)))
	NNHE3(K)=NNHE30(K)*(1.0 + 
     1   0.5*DTANH(SCALMI*(TQM(X,T)/TQM0 -1.0)))
	LPMI(K)=LPMI0(K)*(1.0 + 
     1   0.5*DTANH(SCALMI*(TQM(X,T)/TQM0 -1.0)))
	LPIS(K)=LPIS0(K)*(1.0 + 
     1   0.5*DTANH(SCALIS*(TQM(X,T)/TQM0 -1.0)))
	DO 172 I=1,SOLS
	HMI(K,I)=HMI0(K,I)*(1.0 + 
     1   0.5*DTANH(SCALMI*(TQM(X,T)/TQM0 -1.0)))
	HIS(K,I)=HIS0(K,I)*(1.0 + 
     1   0.5*DTANH(SCALIS*(TQM(X,T)/TQM0 -1.0)))
  172   CONTINUE
	DO 175 I=1,SOLS
	DO 175 J=1,SOLS
	LMI(K,I,J)=LMI0(K,I,J)*(1.0 + 
     1   0.5*DTANH(SCALMI*(TQM(X,T)/TQM0 -1.0)))
	LIS(K,I,J)=LIS0(K,I,J)*(1.0 + 
     1   0.5*DTANH(SCALIS*(TQM(X,T)/TQM0 -1.0)))
  175   CONTINUE
	QIAMM=QIAMM0*(1.0 + 
     1   0.5*DTANH(SCALMI*(TQM(X,T)/TQM0 -1.0)))
