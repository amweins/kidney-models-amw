	SUBROUTINE IMCTNEWT(COUNT,SWOUT)
C
...
C Abolish the interstitial step up at x=1 and x=chop+1
	PS(1,1)=PS(2,1)
	IMPS(1,1)=IMPS(2,1)
	LCHS(1)=LCHS(2)
	PS(CHOP+1,1)=PS(CHOP,1)
	IMPS(CHOP+1,1)=IMPS(CHOP,1)
	LCHS(CHOP+1)=LCHS(CHOP)
	DO 1025 ISOL=1,SOLS
	CS(ISOL,1,1)=CS(ISOL,2,1)
 1025   CS(ISOL,CHOP+1,1)=CS(ISOL,CHOP,1)
	ELSE
	NN=3*(2+SOLS)+2
	ENDIF
C
...
C	PRINT 1082, ITER
 1082   FORMAT(46X,'ITER=',I5,$)
        IF (KX.LT.0.6*CHOP) HMI(1,9) = 1.0D-4*HMI(1,9)
	CALL IMCTFLUX
        IF (KX.LT.0.6*CHOP) HMI(1,9) = 1.0D+4*HMI(1,9)
	CALL IMCTERR(PHI)
	MAXPHI=DABS(PHI(1))
