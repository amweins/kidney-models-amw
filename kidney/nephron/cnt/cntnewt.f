	SUBROUTINE CNTNEWT(COUNT,SWOUT)
C
	INTEGER SOLS,TAU,T,TLIM,NPAR,X,CHOP,COUNT,SWOUT
C
...
C
	...
	IERR=0
C
C START THE CNT CALCULATION
C
	X=1
	IF (T.GT.1) GO TO 1049
	NN=4*(2+SOLS)+1
	REWIND 54
	READ (54,1024) (GAMMA(J),J=1,NN)
 1024   FORMAT (D30.20)
	CALL CNTGAM(1,GAMMA)
	IF (SWOUT.EQ.2) GO TO 1060
C
...
C
 1049   CALL CNTPSET
C	READ(52,1050) DUMDT,DUMFVM,
C     1  DUMVM,VS(X,T),VS(CHOP+X,T),
C     1  DUMPM,PS(X,T),PS(CHOP+X,T),
C     1  DUMIMPM,IMPS(X,T),IMPS(CHOP+X,T),
C     1  (DUMCM,CS(I,X,T),CS(I,CHOP+X,T),I=1,SOLS-1)
 1050   FORMAT (2D12.4,/,3F8.4,/,3F8.4,/,3F8.4,/,(3F14.9))
C
 1060   CONTINUE
        PS(CHOP+1,T)=PS(1,T)
        IMPS(CHOP+1,T)=IMPS(1,T)
        DO 45 I=1,SOLS
   45   CS(I,CHOP+1,T)=CS(I,1,T)
        LCHS(CHOP+1)=LCHS(1)
C        
	DO 1054 J=X,CHOP+X
...
C IF THE SPATIAL ITERATION FAILS TO CONVERGE THE PROGRAM STOPS.
	PRINT 1980
 1980   FORMAT (' TOO MANY ITERATIONS IN CNT')
	WRITE (19,1982) COUNT
 1982   FORMAT ('CNT ERROR IN EXP. ', I3)
	GO TO 9600
C
C THE SPATIAL STEP CONVERGED,AND
C THE PROGRAM PICKS UP AT THIS POINT:
C
 1558   CALL CNRESET(1,CHOP)
 1400   CONTINUE
C
	X=1
	CALL CNTGAM(2,GAMMA)
	X=1+CHOP
	REWIND 55
	WRITE (55,1624) (GAMMA(J),J=1,NN)
 1624   FORMAT(D30.20)
C
C IF THERE IS NO NEED FOR OUTPUT GO TO THE END
	IF ((SWOUT.EQ.0).OR.(SWOUT.EQ.2)) GO TO 9600
C
