...
C
C
	WRITE (51,396) TIME,DIST,AM(X,T),SM(X,T)
  396   FORMAT (1H1,/,5X,'TIME=',F15.4,7X,'DIST=',F8.6,
     1  7X,'AM=',D10.4,7X,'SM=',D10.4)
C
C
	WRITE (51,398) L(X,T),CHVL(X,T),(CLVL(K,X,T),K=1,3),AE(X,T)
	WRITE (51,400) VM(X,T),VE(X,T),(VI(K,X,T),K=1,3),VS(X,T)
	WRITE (51,402) PM(X,T),PE(X,T),(PI(K,X,T),K=1,3),PS(X,T)
C
	WRITE (51,410) (SOL(I),
	...
	WRITE (51,412) SOL(SOLS),
     1  CM(SOLS,X,T),CE(SOLS,X,T),(CI(K,SOLS,X,T),K=1,3),CS(SOLS,X,T)
	WRITE (51,420) IMPM(X,T),(IMP(K,X),K=1,3),IMPS(X,T)
	WRITE (51,430) OSMM,OSME,(OSMI(K),K=1,3),OSMS
	WRITE (51,440) LCHM(X),LCHE(X),(LCHI(K,X),K=1,3),LCHS(X)
	WRITE (51,442) (CBUF(K,X,T),K=1,3),(HCBUF(K,X,T),K=1,3)
C
	WRITE (51,460) (SOL(I),XM(I,X)-XS(I,X),XE(I,X)-XS(I,X),
     1      (XI(K,I,X)-XS(I,X),K=1,3),I=1,SOLS)
	WRITE (51,470) (FIVM(K,X,T),JV(K,X,T),FIVS(K,X,T),K=1,3)
	WRITE (51,480) (SOL(I),
     1  (FIKM(K,I,X,T),JK(K,I,X,T),FIKS(K,I,X,T),K=1,3),I=1,SOLS)
	WRITE (51,485) (CURI(K,X),K=1,3)
	WRITE (51,490) FEVM(X,T),FEVS(X,T),QEV,(QIV(K),K=1,3)
	WRITE (51,500) (SOL(I),FEKM(I,X,T),FEKS(I,X,T),QE(I),
