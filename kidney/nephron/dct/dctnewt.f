	SUBROUTINE DCTNEWT(COUNT,SWOUT)
C
...
C
	...
	IERR=0
C
C START THE DCT CALCULATION
C
	X=1
	IF (T.GT.1) GO TO 1049
	NN=2*(2+SOLS)+1
	REWIND 64
	READ (64,1024) (GAMMA(J),J=1,NN)
 1024   FORMAT (D30.20)
	CALL DCTGAM(1,GAMMA)
...
C IF THE SPATIAL ITERATION FAILS TO CONVERGE THE PROGRAM STOPS.
	PRINT 1980
 1980   FORMAT (' TOO MANY ITERATIONS IN DCT')
	WRITE (19,1982) COUNT
 1982   FORMAT ('DCT ERROR IN EXP. ', I3)
	GO TO 9600
C
C THE SPATIAL STEP CONVERGED,AND
C THE PROGRAM PICKS UP AT THIS POINT:
C
 1558   CALL DCRESET(1,CHOP)
 1400   CONTINUE
C
	X=1
	CALL DCTGAM(2,GAMMA)
