...
C SCALING OF CERTAIN TRANSPORT PARAMETERS FOR A GRID OF SIMULATIONS
C
...
C
	DOUBLE PRECISION RHO(3,20),DRHO(3,20),RHOMIN(3,20)
C
	OPEN (19,FILE='dcparam.tem')
	OPEN (20,FILE='dcparam.dat')
	OPEN (21,FILE='rho.dat')
C
	SOLS=12
C
C  SOLUTE INDEX:
...
C
	...
	DRHO(1,1) = 0.1
	DRHO(2,1) = 0.05
	RHO(1,1) = RHOMIN(1,1) - DRHO(1,1)
	RHO(2,1) = RHOMIN(2,1) - DRHO(2,1)
	DO 60 L=1,20
	RHOMIN(3,L) = 1.0
	DRHO(3,L) = 0.0
	RHO(3,L) = RHOMIN(3,L) - DRHO(3,L)
   60   CONTINUE
	DO 62 L=4,6
	RHOMIN(3,L) = 0.75
	DRHO(3,L) = 0.05
...
C
	DO 200 JSETS=1,26
	RHO(1,1) = RHO(1,1) + DRHO(1,1)
	RHO(2,1) = RHO(2,1) + DRHO(2,1)
	DO 70 L=4,6
   70   RHO(3,L) = RHO(3,L) + DRHO(3,L)
	DO 200 KSETS=1,26
C
...
C
	...
	WRITE (20,125)
     1  AIE(K),AI0(K),CLVL0(K),IMP0(K),ZIMP(K),
     1  KHY(K),KDHY(K),TBUF(K),PKB(K),
     1  RHO(2,1)*NP(K),KNH4(K),NPHK(K),
     1  LHP(K),XIHP(K),XHP(K),
     1  NAE1(K),RHO(1,1)*NTSC(K),NNHE3(K)
	WRITE (20,126)
     1  LPMI(K),LPIS(K),
     1  (SMI(K,I),SIS(K,I),HMI(K,I),HIS(K,I),I=1,SOLS)
...
C
  ...
  136   IF(NIS(K).EQ.0) GO TO 150
	WRITE (20,131) (II(K,LL),JJ(K,LL),RHO(3,LL)*PIS(K,LL),
     1   LL=NMI(K)+1,NMI(K)+NIS(K))
  131   FORMAT (2I5,D12.4)
C
  150   CONTINUE
C
	WRITE (21,190) RHO(1,1)
  190   FORMAT(D16.8)
  200   CONTINUE
	STOP
	END
