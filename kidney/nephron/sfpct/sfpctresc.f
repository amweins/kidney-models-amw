...
C
	...
	DO 120 I=1,SOLS
	DO 120 J=1,SOLS
	CIKM(K,I)=CIKM(K,I)+LMI(K,I,J)*AMI(K)*(XM(J,X)-XI(K,J,X))
	CIKS(K,I)=CIKS(K,I)+LIS(K,I,J)*AIS(K)*(XI(K,J,X)-XS(J,X))
...
C
	DO 140 K=1,1
	DO 140 I=1,SOLS
	PIKM(K,I)=PIKM(K,I)+AMI(K)*ATMI(K,I,X)
	PIKS(K,I)=PIKS(K,I)+AIS(K)*ATIS(K,I,X)
