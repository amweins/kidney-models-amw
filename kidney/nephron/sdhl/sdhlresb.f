...
C
C
	WRITE (121,396) TIME,DIST,AM(X,T),SM(X,T)
  396   FORMAT (1H1,/,5X,'TIME=',F15.4,7X,'DIST=',F8.6,
     1  7X,'AM=',D10.4,7X,'SM=',D10.4)
C
C
	WRITE (121,400) VM(X,T),VS(X,T)
	WRITE (121,402) PM(X,T),PS(X,T)
C
	WRITE (121,410) (SOL(I),
	...
	WRITE (121,412) SOL(SOLS),
     1  CM(SOLS,X,T),CS(SOLS,X,T),XM(SOLS,X)-XS(SOLS,X)
	WRITE (121,420) IMPM(X,T),IMPS(X,T)
	WRITE (121,430) OSMM,OSMS
	WRITE (121,440) LCHM(X),LCHS(X)
	WRITE (121,442) 
C
	WRITE (121,470) (FIVM(K,X,T),K=1,1),FEVM(X,T)
	WRITE (121,480) (SOL(I),(FIKM(K,I,X,T),K=1,1),
     1   FEKM(I,X,T),I=1,SOLS)
	WRITE (121,485) (CURI(K,X),K=1,1),CURE(X)
C
C
