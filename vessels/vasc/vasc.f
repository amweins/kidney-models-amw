...
C
	...
	OPEN (11,FILE='vasc.dat')
	OPEN (12,FILE='xy.fv')
        OPEN (13,FILE='medjcs.dat')
        OPEN (14,FILE='medjcs.sum')
C
...
C
C
        READ (10,25) (RADVR(I),RDNUM(I),I=1,8)
   25   FORMAT(2D12.4)
        DO 26 I=1,8
...
C
        DIST = 0.D0
	CALL ARCHIVR(1,IVR,1)
	CALL OMDRESD(RDNUM(IVR))
C
	CALL ARCHIVR(1,IVR,4)
	CALL OMARESD(RANUM(IVR))
C
  190   CONTINUE
C
...
C
        DIST = 0.D0
	CALL ARCHIVR(1,IVR,1)
	CALL OIDRESD(RDNUM(IVR))
C
	CALL ARCHIVR(1,IVR,2)
	CALL IMDRESD(RDNUM(IVR))
C
	CALL ARCHIVR(1,IVR,3)
	CALL IMARESD(RANUM(IVR))
C
	CALL ARCHIVR(1,IVR,4)
	CALL OIARESD(RANUM(IVR))
C
  290   CONTINUE
C
...
C
        DIST = 0.D0
	CALL ARCHIVR(1,IVR,1)
	CALL MRDRESD(RDNUM(IVR))
C
	CALL ARCHIVR(1,IVR,4)
	CALL MRARESD(RANUM(IVR))
C
C SUMMATION OF VASCULAR FLUXES FOR EACH LEVEL OM -> IM.
C
        CALL VGEN       
