	SUBROUTINE IMDNEWT(COUNT,IVR,SWOUT)
C
...
C
	...
	TIME=0.D0
   40   T=T+1
	READ(222,50) DT,DUMFVC,DUMHCTC,(POX0(I),I=1,SLICE+1)
	READ(222,51) DUMVC,VS(1,T),VS(CHOP+1,T),
...
C
	IF (T.EQ.1) CALL IMDRESA
	CALL IMDRESB
	IF (TAU.EQ.1) GO TO 570
C
C  THUS IN THE STEADY STATE CASE WE ARE DONE.
	PRINT 560, COUNT
  560   FORMAT (' PROBLEM ',I2,' SOLVED FOR THE IMDVR')
	WRITE(224,565) DT,FVC(CHOP+1,T),HCTC(CHOP+1,T),SAT(CHOP+1,T),
