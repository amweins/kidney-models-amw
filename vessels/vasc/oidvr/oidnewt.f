	SUBROUTINE OIDNEWT(COUNT,IVR,SWOUT)
C
...
C
	...
	TIME=0.D0
   40   T=T+1
	READ(262,50) DT,FVC(1,T),HCTC(1,T),(POX0(I),I=1,SLICE+1)
	READ(262,51) VC(1,T),VS(1,T),VS(CHOP+1,T),
     1  PC(1,T),PS(1,T),PS(CHOP+1,T),
     1  CC(16,1,T),CS(16,1,T),CS(16,CHOP+1,T),
     1  (CC(I,1,T),CS(I,1,T),CS(I,CHOP+1,T),I=1,11),
     1  (CC(I,1,T),CS(I,1,T),CS(I,CHOP+1,T),I=13,15)
...
C IN THIS CASE IVR = 3,7 AND OIDVR IS THE FULL THICKNESS OF OM
C
...
C
   57   X=0
	...
	FBC(1,T)=FVC(1,T)/(1.D0-HCTC(1,T))
	DO 400 KX=1,CHOP+1
	DIST= 0.D0 + CL*FLOAT(KX-1)/FLOAT(CHOP)
...
C
	IF (T.EQ.1) CALL OIDRESA
	CALL OIDRESB
	IF (TAU.EQ.1) GO TO 570
C
C  THUS IN THE STEADY STATE CASE WE ARE DONE.
	PRINT 560, COUNT
  560   FORMAT (' PROBLEM ',I2,' SOLVED FOR THE OIDVR')
	WRITE(264,565) DT,FVC(CHOP+1,T),HCTC(CHOP+1,T),SAT(CHOP+1,T),
