	SUBROUTINE OIANEWT(COUNT,IVR,SWOUT)
C
...
C IN THIS CASE IVR = 3,7 AND OIAVR IS THE FULL THICKNESS OF OM
C
...
C
   ...
   40   T=T+1
        X=1
	READ(272,50) DT,DUMFVC,DUMHCTC,(POX0(I),I=1,SLICE+1)
	READ(272,51) DUMVC,VS(X,T),VS(CHOP+X,T),
...
C
	IF (T.EQ.1) CALL OIARESA
	CALL OIARESB
	IF (TAU.EQ.1) GO TO 570
C
C  THUS IN THE STEADY STATE CASE WE ARE DONE.
	PRINT 560, COUNT
  560   FORMAT (' PROBLEM ',I2,' SOLVED FOR THE OIAVR')
	WRITE(274,565) DT,FVC(CHOP+1,T),HCTC(CHOP+1,T),SAT(CHOP+1,T),
