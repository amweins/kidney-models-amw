	SUBROUTINE OMDNEWT(COUNT,IVR,SWOUT)
C
...
C
	...
	TIME=0.D0
   40   T=T+1
	READ(202,50) DT,FVC(1,T),HCTC(1,T),(POX0(I),I=1,SLICE+1)
	READ(202,51) VC(1,T),VS(1,T),VS(CHOP+1,T),
...
C
   57   X=0
	DO 400 KX=1,CHOP+1
	DIST=0.0 + CL*FLOAT(KX-1)/FLOAT(CHOP)
...
C
	IF (T.EQ.1) CALL OMDRESA
	CALL OMDRESB
	IF (TAU.EQ.1) GO TO 570
C
C  THUS IN THE STEADY STATE CASE WE ARE DONE.
	PRINT 560, COUNT
  560   FORMAT (' PROBLEM ',I2,' SOLVED FOR THE OMDVR')
	WRITE(204,565) DT,FVC(CHOP+1,T),HCTC(CHOP+1,T),SAT(CHOP+1,T),
