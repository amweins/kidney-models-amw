...
C
   ...
   40   T=T+1
        X=1
	READ(212,50) DT,DUMFVC,DUMHCTC,(POX0(I),I=1,SLICE+1)
	READ(212,51) DUMVC,VS(X,T),VS(CHOP+X,T),
...
C
	...
	POX((IS-1)*CHOP+J,T)=
     1   (1.D0-RJ)*POX0(2-IVR+IS) + RJ*POX0(2-IVR+IS+1)
   44   CONTINUE
   45   CONTINUE
C
...
C  THUS IN THE STEADY STATE CASE WE ARE DONE.
  ...
  560   FORMAT (' PROBLEM ',I2,' SOLVED FOR THE OMAVR')
	WRITE(214,565) DT,FVC(CHOP+1,T),HCTC(CHOP+1,T),SAT(CHOP+1,T),
