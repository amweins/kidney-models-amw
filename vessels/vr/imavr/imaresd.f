...
C
C
	DOUBLE PRECISION RADVR,CURC(901),OSMC(901),OSMS(901),
...
C
	DO 240 IX = 1,CHOP+1
        FVC(IX,T) = FVC(IX,T)*RADVR
        FVCS(IX,T) = FVCS(IX,T)*RADVR
        FBC(IX,T) = FBC(IX,T)*RADVR
	DO 238 ISOL = 1,SOLS
        FKC(ISOL,IX,T) = FKC(ISOL,IX,T)*RADVR
        FKCS(ISOL,IX,T) = FKCS(ISOL,IX,T)*RADVR
  238   CONTINUE
...
C
  ...
  710   WRITE (12,715) DIST,1.D6*FVC(KX,T),
     ...
     1   1.D3*(OSMC(KX)-OSMS(KX)),1.D6*FVCS(KX,T),
     1   1.D6*RADVR*FVCSE(KX,T), 1.D6*RADVR*FVCSI(KX,T)
  715   FORMAT(8D16.8)
	RETURN
	END
