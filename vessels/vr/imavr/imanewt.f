	SUBROUTINE IMANEWT(COUNT,IVR,SWOUT)
C
...
C
   ...
   40   T=T+1
        X=1
	READ(232,50) DT,DUMFVC,DUMHCTC,(POX0(I),I=1,SLICE+1)
	READ(232,51) DUMVC,VS(X,T),VS(CHOP+X,T),
...
C
	...
	POX((IS-1)*CHOP+J,T)=
     1   (1.D0-RJ)*POX0(7-IVR+IS) + RJ*POX0(7-IVR+IS+1)
   44   CONTINUE
   45   CONTINUE
C
C
C RESET THE INITIAL PERITUBULAR VALUES FOR THE SHORTER LOOPS
        VS(X,T) = VS(X,T) +
     1   (1.-FLOAT(IVR-2)/5.)*(VS(CHOP+X,T)-VS(X,T))
        PS(X,T) = PS(X,T) +
     1   (1.-FLOAT(IVR-2)/5.)*(PS(CHOP+X,T)-PS(X,T))
        IMPS(X,T) = IMPS(X,T) +
     1   (1.-FLOAT(IVR-2)/5.)*(IMPS(CHOP+X,T)-IMPS(X,T))
        DO 46 I=1,SOLS-1
   46   CS(I,X,T) = CS(I,X,T) +
     1   (1.-FLOAT(IVR-2)/5.)*(CS(I,CHOP+X,T)-CS(I,X,T))
C
C
	DO 54 J=1,CHOP+1
	RJ=DBLE(FLOAT(J-1)/FLOAT(CHOP))
...
C
	IF (T.EQ.1) CALL IMARESA
	CALL IMARESB
	IF (TAU.EQ.1) GO TO 570
C
C  THUS IN THE STEADY STATE CASE WE ARE DONE.
	PRINT 560, COUNT
  560   FORMAT (' PROBLEM ',I2,' SOLVED FOR THE IMAVR')
	WRITE(234,565) DT,FVC(CHOP+1,T),HCTC(CHOP+1,T),SAT(CHOP+1,T),
