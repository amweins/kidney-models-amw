	SUBROUTINE MRDNEWT(COUNT,IVR,SWOUT)
C
...
C
	...
	TIME=0.D0
   40   T=T+1
	READ(242,50) DT,FVC(1,T),HCTC(1,T),(POX0(I),I=1,SLICE+1)
	READ(242,51) VC(1,T),VS(1,T),VS(CHOP+1,T),
...
C
	IF (T.EQ.1) CALL MRDRESA
	CALL MRDRESB
	IF (TAU.EQ.1) GO TO 570
C
C  THUS IN THE STEADY STATE CASE WE ARE DONE.
	PRINT 560, COUNT
  560   FORMAT (' PROBLEM ',I2,' SOLVED FOR THE MRDVR')
	WRITE(244,565) DT,FVC(CHOP+1,T),HCTC(CHOP+1,T),SAT(CHOP+1,T),
