...
C
	...
	DO 44 LL=1,NMI
	READ(20,42) ORDMI(LL),RMI(LL)
	DO 44 KK=1,ORDMI(LL)
   44   READ(20,31) IMI(KK,LL),JMI(KK,LL),MMI(KK,LL)
C
   36   IF(NIS.EQ.0) GO TO 37
	DO 46 LL=1,NIS
	READ(20,42) ORDIS(LL),RIS(LL)
	DO 46 KK=1,ORDIS(LL)
   46   READ(20,31) IIS(KK,LL),JIS(KK,LL),MIS(KK,LL)
   31   FORMAT (3I5)
   42   FORMAT (I5,D12.4)
   37   CONTINUE
C
...
C
	...
	LMI(K,IMI(KK,LL),JMI(KK,LL))=
     1    LMI(K,IMI(KK,LL),JMI(KK,LL)) + FLOAT(MMI(KK,LL))*RMI(LL)
  144   LMI(K,JMI(KK,LL),IMI(KK,LL))=LMI(K,IMI(KK,LL),JMI(KK,LL))
C
  136   IF(NIS.EQ.0) GO TO 150
	DO 146 LL=1,NIS
	DO 146 KK=1,ORDIS(LL)
	LIS(K,IIS(KK,LL),JIS(KK,LL))=
     1    LIS(K,IIS(KK,LL),JIS(KK,LL)) + FLOAT(MIS(KK,LL))*RIS(LL)
  146   LIS(K,JIS(KK,LL),IIS(KK,LL))=LIS(K,IIS(KK,LL),JIS(KK,LL))
C
  150   CONTINUE
C
	RETURN
