...
C
	double precision dhmi,dlmi,dlis
C
...
C
C	hmi(2,1)=float(nsets-ksets+1)*dhmi
C	hmi(2,2)=float(nsets-ksets+1)*dhmi
C
	hmi(2,1)=.15d-4
	hmi(2,2)=.15d-4
C
	pmi(2,1)=float(ksets)*dlmi
	pmi(2,2)=float(ksets)*dlmi
	pmi(2,3)=float(ksets)*dlmi
C
	pis(2,10)=float(ksets)*dlis
	pis(2,11)=float(ksets)*dlis
	pis(2,12)=float(ksets)*dlis
C
...
C
	...
	WRITE (20,126)
     1  LPMI(K),LPIS(K),
     1  (SMI(K,I),SIS(K,I),HMI(K,I),HIS(K,I),I=1,SOLS)
  125   FORMAT (5D12.4,/,4D12.4,/,3D12.4,/,3D12.4)
