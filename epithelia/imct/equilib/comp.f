...
C 
C DETERMINES LUMINAL NACL CONCENTRATION WHICH YIELDS ZERO NA FLUX 
C
...
C
	DOUBLE PRECISION GAMMA(60),
     1   DCM,PHIFK,DFDCM,EPSFK
C
C	PHIFK-	ERROR BETWEEN CALCULATED AND DESIRED FLUXES
C	DFDCM-	NUMERICAL DERIVATIVE OF NA FLUX WRT CM(NA)
C	DCM-	INCREMENT TO COMPUTE NUMERICAL DERIVATIVE 
...
C
	DCM=1.D-2*CM(1)
 	DFDCM=FIKM(1,1,T)+FEKM(1,T)
C 	DFDCM=FIKM(1,1,T)
	CM(1)=CM(1)+DCM
	CM(3)=CM(3)+DCM
	CALL NEWTON(NN,GAMMA)
	DFDCM=(FIKM(1,1,T)+FEKM(1,T)-DFDCM)/DCM
C	DFDCM=(FIKM(1,1,T)-DFDCM)/DCM
	CM(1)=CM(1)-DCM
	CM(3)=CM(3)-DCM
	CM(1)=CM(1) - PHIFK/DFDCM
	CM(3)=CM(3) - PHIFK/DFDCM
C
