...
C
   ...
   60   FORMAT(' NUMBER OF PARAMETER SETS NEEDED=',$)
	READ *, NSETS
C
	dlis=pis(2,4)/float(nsets)
C
C
	DO 200 KSETS=1,NSETS+1
C
	pis(2,4)=float(nsets+1-ksets)*dlis
	pis(2,5)=float(nsets+1-ksets)*dlis
	pis(2,6)=float(nsets+1-ksets)*dlis
C
...
C
  150   CONTINUE
C
	write (18,199) pis(2,4)/(float(nsets)*dlis)
  199   format (d16.8)
  200   CONTINUE
	STOP
	END
