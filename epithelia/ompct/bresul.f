...
C
	WRITE (21,398) L(T),CHVL(T),(CLVL(K,T),K=1,2),AE(T)
	WRITE (21,400) VM,VE(T),(VI(K,T),K=1,2),VS
	WRITE (21,402) PM,PE(T),(PI(K,T),K=1,2),PS
C
	WRITE (21,410) (SOL(I),
     1  CM(I),CE(I,T),(CI(K,I,T),K=1,2),CS(I),I=1,4)	
	WRITE (21,412) (SOL(I),
     1  CM(I),CE(I,T),(CI(K,I,T),K=1,2),CS(I),I=5,5)	
	WRITE (21,414) (SOL(I),
     1  CM(I),CE(I,T),(CI(K,I,T),K=1,2),CS(I),I=6,SOLS-3)	
	WRITE (21,412) (SOL(I),
     1  CM(I),CE(I,T),(CI(K,I,T),K=1,2),CS(I),I=SOLS-2,SOLS-2)	
	WRITE (21,414) (SOL(I),
     1  CM(I),CE(I,T),(CI(K,I,T),K=1,2),CS(I),I=SOLS-1,SOLS-1)	
	WRITE (21,412) SOL(SOLS),
     1  CM(SOLS),CE(SOLS,T),(CI(K,SOLS,T),K=1,2),CS(SOLS)
	WRITE (21,420) IMPM,(IMP(K),K=1,2),IMPS
	WRITE (21,430) OSMM,OSME,(OSMI(K),K=1,2),OSMS
	WRITE (21,440) LCHM,LCHE,(LCHI(K),K=1,2),LCHS
	WRITE (21,442) (CBUF(K,T),K=1,2),(HCBUF(K,T),K=1,2)
C
	WRITE (21,460) (SOL(I),XM(I)-XS(I),XE(I)-XS(I),
     1      (XI(K,I)-XS(I),K=1,2),I=1,SOLS)
	WRITE (21,470) (FIVM(K,T),JV(K,T),FIVS(K,T),K=1,2)
	WRITE (21,480) (SOL(I),
     1  (FIKM(K,I,T),JK(K,I,T),FIKS(K,I,T),K=1,2),I=1,SOLS)
	WRITE (21,485) (CURI(K),K=1,2)
	WRITE (21,490) FEVM(T),FEVS(T),QEV,(QIV(K),K=1,2)
	WRITE (21,500) (SOL(I),FEKM(I,T),FEKS(I,T),QE(I),
     1  (QI(K,I),K=1,2),I=1,SOLS)
	WRITE (21,505) CURE
C
C
  398   FORMAT(//,14X,'L',8X,'CHVL',7X,'CLVLP',7X,'CLVLA',
     1  9X,'AE',/,6X,5D12.4)
  400   FORMAT(//,16X,'VM',10X,'VE',9X,'VIP',9X,'VIA',
     1  10X,'VS',/,8X,5F12.3)
  402   FORMAT(//,16X,'PM',10X,'PE',9X,'PIP',9X,'PIA',
     1  10X,'PS',/,8X,5F12.3)
C
  410   FORMAT(//,16X,'CM',10X,'CE',9X,'CIP',9X,'CIA',
     1  10X,'CS',/,(4X,A5,5F12.6))
  412   FORMAT(4X,A5,1X,5D12.4)
  414   FORMAT(4X,A5,5F12.6)
  420   FORMAT(5X,'IMP',1X,F12.6,12X,3F12.6)
  430   FORMAT(5X,'OSM',1X,5F12.6)
  440   FORMAT(6X,'PH',1X,5F12.6)
  442   FORMAT(4X,'CBUF',1X,24X,2F12.6,/,4X,'HCBUF',24X,2F12.6)
C
  460   FORMAT(//,15X,'XM-XS',7X,'XE-XS',6X,'XIP-XS',
     1  6X,'XIA-XS',/,(4X,A5,4F12.4))
  ...
  480   FORMAT(//,14X,'FIKMP',7X,'JKP',9X,'FIKSP',7X,
     ...
     1  /,(4X,A5,6D12.4))
  485   FORMAT(4X,'CUR',2X,D12.4,24X,D12.4)
  490   FORMAT(//,15X,'FEVM',8X,'FEVS',20X,'QEV',9X,
     1  'QIVP',8X,'QIVA',/,9X,2D12.4,12X,3D12.4)
  500   FORMAT(//,15X,'FEKM',8X,'FEKS',20X,'QE',10X,
     1  'QIP',9X,'QIA',/,(4X,A5,2D12.4,12X,3D12.4))
...
C These are as needed.
	WRITE (23,570) 1.D6*HMI(2,2)*AI0(2),
     1   1.D3*CM(1),1.D3*CM(2),1.D3*CM(4),VM,
     1   1.D6*FEKM(1,T),1.D6*FEKM(2,T),
     1   1.D6*FIKM(1,1,T),1.D6*FIKM(1,2,T),
     1   1.D4*CLVL(2,T), VI(2,T),
     1   1.D6*(FIKM(2,2,T)-ATMI(2,2)*AI0(2)),
     1   1.D6*ATMI(2,2)*AI0(2),-1.D6*ATMI(2,12)*AI0(2)
C     1  (1.D6*FEKM(I,T),I=1,2),1.D6*(FEKM(4,T)-FEKM(12,T)),
  570   FORMAT (F6.2,3F6.0,F6.1,4F8.3,F6.2,F6.1,3F8.3)
C  570   FORMAT (8E16.8)
C
	RETURN
	END
