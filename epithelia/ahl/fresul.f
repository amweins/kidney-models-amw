...
C
C THIS PORTION BACK-CALCULATES INDIVIDUAL NET COTRANSPORTERS 
C
C	JNAKCL,JNAHM,JKCL,JNAHS,JCLB,JNAB,JNAP
C
	JNAHM=-CIKM(1,12)
	JNAKCL=CIKM(1,2)
	JNAHS= CIKS(1,12)+CJK(1,12)
	JNAP= CIKS(1,7)+CJK(1,7)
	JKCL= CIKS(1,2)+CJK(1,2)
	JCLB= CIKS(1,3)+CJK(1,3) - JKCL
	JNAB= CIKS(1,1)+CJK(1,1) - 2.*JNAP + JNAHS
C
  125   FORMAT(D12.4)
	WRITE (JCOTR(1,8),125) JNAKCL
	WRITE (JCOTR(2,8),125) JNAKCL
	WRITE (JCOTR(3,8),125) 2.*JNAKCL
	WRITE (JCOTR(1,9),125) JNAHM
	WRITE (JCOTR(12,9),125) -JNAHM
	WRITE (JCOTR(2,10),125) JKCL
	WRITE (JCOTR(3,10),125) JKCL
	WRITE (JCOTR(1,4),125) -JNAHS
	WRITE (JCOTR(12,4),125) JNAHS
	...
	WRITE (JCOTR(4,7),125) 3.*JNAB
C
C NKCC EXCHANGER
C
	WRITE (JCOTR(1,1),125) JNKCC(1,1)*SCALE
	WRITE (JCOTR(2,1),125) JNKCC(1,2)*SCALE
	WRITE (JCOTR(3,1),125) JNKCC(1,3)*SCALE
	WRITE (JCOTR(11,1),125) JNKCC(1,4)*SCALE
C
	CIKM(1,1) = CIKM(1,1) + JNKCC(1,1)*SCALE
	CIKM(1,2) = CIKM(1,2) + JNKCC(1,2)*SCALE
	CIKM(1,3) = CIKM(1,3) + JNKCC(1,3)*SCALE
	CIKM(1,11) = CIKM(1,11) + JNKCC(1,4)*SCALE
C
C KCC EXCHANGER
C
	WRITE (JCOTR(2,3),125) JKCC(1,1)*SCALE
	WRITE (JCOTR(3,3),125) JKCC(1,2)*SCALE
	WRITE (JCOTR(11,3),125) JKCC(1,3)*SCALE
C
	CJK(1,2) = CJK(1,2) + JKCC(1,1)*(AIE(1)/(AIE(1)+AI0(1)))*SCALE
	CJK(1,3) = CJK(1,3) + JKCC(1,2)*(AIE(1)/(AIE(1)+AI0(1)))*SCALE
	CJK(1,11) = CJK(1,11) + JKCC(1,3)*(AIE(1)/(AIE(1)+AI0(1)))*SCALE
C
	CIKS(1,2) = CIKS(1,2) + JKCC(1,1)*(AI0(1)/(AIE(1)+AI0(1)))*SCALE
	CIKS(1,3) = CIKS(1,3) + JKCC(1,2)*(AI0(1)/(AIE(1)+AI0(1)))*SCALE
	CIKS(1,11) = CIKS(1,11) + JKCC(1,3)*(AI0(1)/(AIE(1)+AI0(1)))*SCALE
C
C NAH EXCHANGER
C
...
C
  ...
  140   PJK(K,I)=PJK(K,I)+AIE(K)*ATIE(K,I)*SCALE
C
	DO 150 K=1,1
	DO 150 I=1,SOLS
...
C
  480   FORMAT(/,14X,'CIKMA',7X,'CIKSA',7X,
     1   'JNKCC',8X,'JNHE3',7X,'JKCC',8X,'JNAHS',7X,'JCLB',8X,
     1   'JNAP',8X,'JNAB',8X,
     1   /,(4X,A5,2D12.4,7A12))
...
C	GO TO 501
C	IF (II.EQ.2) WRITE (23,500) 1.D3*CM(11),
C     1    FLUXS(1),FEKM(1,T),FIKM(1,1,T), 
C     1    FLUXS(2),SCALE*JNKCC(1,2),GIKM(1,2),
C     1    FLUXS(11),FLUXS(10)
C	IF (II.EQ.2) WRITE (28,500) 1.D3*CM(11),
C     1    SCALE*JNKCC(1,1),SCALE*JNKCC(1,2),SCALE*JNKCC(1,4),
C     1    SCALE*JNHE3(1,1),SCALE*JNHE3(1,2),SCALE*JNHE3(1,3),
C     1    PJK(1,1)+PIKS(1,1),PJK(1,2)+PIKS(1,2),PJK(1,11)+PIKS(1,11),
C     1    SCALE*JKCC(1,1),SCALE*JKCC(1,2),SCALE*JKCC(1,3)
C	IF (II.EQ.2) WRITE (29,500) 1.D3*CM(11),1.D4*CLVL(1,T),
C     1    1.D3*CI(1,1,T),1.D3*CI(1,2,T),1.D3*CI(1,3,T),
C     1    1.D3*CI(1,4,T),1.D3*CI(1,11,T)
	IF (II.EQ.2) WRITE (23,500) TIME,
     1    FIKM(1,1,T), FIKM(1,12,T), GIKM(1,12),
     1    SCALE*JNKCC(1,1),SCALE*JNHE3(1,1),SCALE*JNHE3(1,2)
	IF (II.EQ.2) WRITE (28,500) TIME,
     1    VM,VI(1,T),VM-VI(1,T),
     1    LCHM,LCHI(1),LCHI(1)-LCHM,XM(12)-XI(1,12)
	IF (II.EQ.2) WRITE (29,500) TIME,1.D4*CLVL(1,T),
     1    1.D3*CI(1,1,T),1.D3*CI(1,3,T),
     1    1.D3*CI(1,4,T),1.D3*CI(1,11,T),LCHI(1)
  500   FORMAT (8D16.8)
  501   CONTINUE
C
	FEVM(T)=FEVM(T)/SCALE
