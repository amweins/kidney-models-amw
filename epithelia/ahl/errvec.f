...
C
	JNKCC(1,1)=NNKCC(1)*AI0(1)*VNKCC(1)
	JNKCC(1,2)=NNKCC(1)*AI0(1)*VNKCC(2)
	JNKCC(1,3)=NNKCC(1)*AI0(1)*VNKCC(3)
	JNKCC(1,4)=NNKCC(1)*AI0(1)*VNKCC(4)
C
	FIKM(1,1,T)=FIKM(1,1,T) + JNKCC(1,1)
	FIKM(1,2,T)=FIKM(1,2,T) + JNKCC(1,2)
	FIKM(1,3,T)=FIKM(1,3,T) + JNKCC(1,3)
	FIKM(1,11,T)=FIKM(1,11,T) + JNKCC(1,4)
C
C THE KCC EXCHANGER
C
C Transported Solutes:
...
C
	JKCC(1,1)=NKCL(1)*AIE(1)*VKCC(1)
	JKCC(1,2)=NKCL(1)*AIE(1)*VKCC(2)
	JKCC(1,3)=NKCL(1)*AIE(1)*VKCC(3)
C
	JK(1,2,T)=JK(1,2,T) + NKCL(1)*AIE(1)*VKCC(1)
	JK(1,3,T)=JK(1,3,T) + NKCL(1)*AIE(1)*VKCC(2)
	JK(1,11,T)=JK(1,11,T) + NKCL(1)*AIE(1)*VKCC(3)
C
C
	SKCC(1)=CS(2)
	SKCC(3)=CS(3)
...
C
	...
	VKCC(2)=VKCC(1)+VKCC(3)
C
	JKCC(1,1)=JKCC(1,1) + NKCL(1)*AI0(1)*VKCC(1)
	JKCC(1,2)=JKCC(1,2) + NKCL(1)*AI0(1)*VKCC(2)
	JKCC(1,3)=JKCC(1,3) + NKCL(1)*AI0(1)*VKCC(3)
C
	FIKS(1,2,T)=FIKS(1,2,T) + NKCL(1)*AI0(1)*VKCC(1)
	FIKS(1,3,T)=FIKS(1,3,T) + NKCL(1)*AI0(1)*VKCC(2)
	FIKS(1,11,T)=FIKS(1,11,T) + NKCL(1)*AI0(1)*VKCC(3)
C
C PUMPS
C
C  SODIUM PUMPS
C
	DO 125 K=1,1
        ...
        KNPK(K)=0.0001*(1.D0+CE(1,T)/.0185)
        ATIE(K,1)=NP(K)*(CI(K,1,T)/(KNPN(K)+CI(K,1,T)))**3*
     1    (CE(2,T)/(KNPK(K)+CE(2,T)))**2
...
C  ALLOW FOR COMPETITION BETWEEN K+ AND NH4+ 
	ATIE(K,2)=-ATIE(K,1)*0.667*CE(2,T)/(CE(2,T)+CE(11,T)/KNH4(K))
	ATIE(K,11)=-ATIE(K,1)*0.667*CE(11,T)/(KNH4(K)*CE(2,T)+CE(11,T))
	ATIS(K,2)=-ATIS(K,1)*0.667*CS(2)/(CS(2)+CS(11)/KNH4(K))
	ATIS(K,11)=-ATIS(K,1)*0.667*CS(11)/(KNH4(K)*CS(2)+CS(11))
C
	JK(K,1,T)=JK(K,1,T)+AIE(K)*ATIE(K,1)
	JK(K,2,T)=JK(K,2,T)+AIE(K)*ATIE(K,2)
	JK(K,11,T)=JK(K,11,T)+AIE(K)*ATIE(K,11)
	FIKS(K,1,T)=FIKS(K,1,T)+AI0(K)*ATIS(K,1)
	FIKS(K,2,T)=FIKS(K,2,T)+AI0(K)*ATIS(K,2)
	FIKS(K,11,T)=FIKS(K,11,T)+AI0(K)*ATIS(K,11)
C
	JNAK(K,1)= AIE(K)*ATIE(K,1)+AI0(K)*ATIS(K,1)
	JNAK(K,2)= AIE(K)*ATIE(K,2)+AI0(K)*ATIS(K,2)
	JNAK(K,3)= AIE(K)*ATIE(K,11)+AI0(K)*ATIS(K,11)
  125   CONTINUE
C
C  PROTON PUMPS
C
