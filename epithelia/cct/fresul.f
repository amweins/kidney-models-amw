...
C
	...
	SS(4)=CE(4,T)
	CALL AE1(VAE1,SS)
	CJK(2,3)=CJK(2,3)-NAE1(2)*AIE(2)*VAE1*SCALE
	CJK(2,4)=CJK(2,4)+NAE1(2)*AIE(2)*VAE1*SCALE
	SS(3)=CS(3)
	SS(4)=CS(4)
	CALL AE1(VAE1,SS)
	CIKS(2,3)=CIKS(2,3)-NAE1(2)*AI0(2)*VAE1*SCALE
	CIKS(2,4)=CIKS(2,4)+NAE1(2)*AI0(2)*VAE1*SCALE
C
	SS(1)=CM(3)
	SS(2)=CM(4)
	SS(3)=CI(3,3,T)
	SS(4)=CI(3,4,T)
	CALL AE1(VAE1,SS)
	FIKM(3,3,T)=FIKM(3,3,T)-NAE1(3)*AI0(3)*VAE1*SCALE
	FIKM(3,4,T)=FIKM(3,4,T)+NAE1(3)*AI0(3)*VAE1*SCALE
C
C
C PUMPS
C
...
C	IF (II.EQ.2) WRITE (23,570) VM,
C     1   FLUXS(1),GIKM(1,1),FEKM(1,T),
C     1   FLUXS(3),CIKM(2,3),CIKM(3,3),FEKM(3,T),
C     1   FLUXS(12),PIKM(2,12),CIKM(2,12)
	IF (II.EQ.2) WRITE (23,570) VM,
     1   FLUXS(1),GIKM(1,1),FEKM(1,T),
     1   FLUXS(3),-CIKM(3,4),CIKM(3,3),FEKM(3,T),
     1   FLUXS(12),PIKM(2,12),CIKM(3,12)
  570   FORMAT (8E16.8)
C
	FEVM(T)=FEVM(T)/SCALE
	FEVS(T)=FEVS(T)/SCALE
