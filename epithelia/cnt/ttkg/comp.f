...
C REVISED AGAIN 2/04 TO BE USED AS A CONNECTING SEGMENT MODEL
C 
C TO FIND THE TTKG, THE PROGRAM VARIES LUMINAL KCL CONCENTRATION
C
C
...
C
	DOUBLE PRECISION GAMMA(60),
     1   DCM,PHIJK,DJKDCM,EPSJK
C
C	PHIJK-	ERROR BETWEEN CALCULATED AND DESIRED VOLUME
C	DJKDCM-	NUMERICAL DERIVATIVE OF CELL VOLUME WRT HIE(K)
C	DCM-	INCREMENT IN CM(K) TO COMPUTE NUMERICAL DERIVATIVE 
C	EPSJK-	CONVERGENCE CRITERION
C
C
        COMMON SOLS,T,
     1   Z,RT,RTE,F,
...
C
   ...
   50   FORMAT (D12.4,/,6F8.4,/,(2F14.9))
	...
	IF (COUNT.GT.1) CM(3)=CM(3) + DCM - CM(2)
	IF (COUNT.GT.1) CM(2)=DCM
...
C
	PHIJK=FEKM(2,T)
	DO 90 K=1,3
   90   PHIJK=PHIJK + FIKM(K,2,T)
C
	PRINT 82, ITER
   82   FORMAT('FITITER=',I5,$)
	PRINT 232,PHIJK
  232   FORMAT (D12.4)
	IF (DABS(PHIJK).LT.EPSJK) GO TO 558
C
C UPDATE THE LUMEN CONCENTRATIONS
C
	DCM=1.D-2*CM(2)
	CM(2)=CM(2)+DCM
	CM(3)=CM(3)+DCM
	...
	DJKDCM=FEKM(2,T)
	DO 95 K=1,3
   95   DJKDCM=DJKDCM + FIKM(K,2,T)
	DJKDCM=(DJKDCM - PHIJK)/DCM
	CM(2)=CM(2) - DCM - PHIJK/DJKDCM
	CM(3)=CM(3) - DCM - PHIJK/DJKDCM
C
