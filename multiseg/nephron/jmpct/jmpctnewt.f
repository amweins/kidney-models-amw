	SUBROUTINE JMPCTNEWT(COUNT,SWOUT)
C
...
C
	...
	NN=2*(2+SOLS)+1
	REWIND 144
	READ (144,24) (GAMMA(J),J=1,NN)
   24   FORMAT (D30.20)
	CALL JMPCTGAM(1,GAMMA)
...
C
   49   CALL JMPCTPSET
	READ(142,50) DT,FVM(1,T),
     1  DUMVM,VS(1,T),VS(CHOP+1,T),
     1  DUMPM,PS(1,T),PS(CHOP+1,T),
...
C
	...
	IF (TAU.EQ.1) CALL JMPCTPTIM(NPAR)
	CALL JMPCTORQUE(1)
C
...
C IF THE SPATIAL ITERATION FAILS TO CONVERGE THE PROGRAM STOPS.
	PRINT 980
  980   FORMAT (' TOO MANY ITERATIONS IN JMPCT')
	WRITE (19,982) COUNT
  982   FORMAT ('JMPCT ERROR IN EXP. ', I3)
	GO TO 600
C
C THE SPATIAL STEP CONVERGED,AND
C THE PROGRAM PICKS UP AT THIS POINT:
C
  558   CALL JMPCRESET(1)
  400   CONTINUE
C
	X=1
	CALL JMPCTGAM(2,GAMMA)
