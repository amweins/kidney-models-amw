...
C
C
	WRITE (161,396) TIME,DIST,AM(X,T),SM(X,T)
  396   FORMAT (1H1,/,5X,'TIME=',F15.4,7X,'DIST=',F8.6,
     1  7X,'AM=',D10.4,7X,'SM=',D10.4)
C
C
	WRITE (161,398) L(X,T),CHVL(X,T),(CLVL(K,X,T),K=1,1),AE(X,T)
	WRITE (161,400) VM(X,T),VE(X,T),(VI(K,X,T),K=1,1),VS(X,T)
	WRITE (161,402) PM(X,T),PE(X,T),(PI(K,X,T),K=1,1),PS(X,T)
C
	WRITE (161,410) (SOL(I),
	...
	WRITE (161,420) IMPM(X,T),(IMP(K,X),K=1,1),IMPS(X,T)
	WRITE (161,430) OSMM,OSME,(OSMI(K),K=1,1),OSMS
	WRITE (161,440) LCHM(X),LCHE(X),(LCHI(K,X),K=1,1),LCHS(X)
	WRITE (161,442) (CBUF(K,X,T),K=1,1),(HCBUF(K,X,T),K=1,1)
C
	WRITE (161,460) (SOL(I),XM(I,X)-XS(I,X),XE(I,X)-XS(I,X),
     1      (XI(K,I,X)-XS(I,X),K=1,1),I=1,SOLS)
	WRITE (161,470) (FIVM(K,X,T),JV(K,X,T),FIVS(K,X,T),K=1,1)
	WRITE (161,480) (SOL(I),
     1  (FIKM(K,I,X,T),JK(K,I,X,T),FIKS(K,I,X,T),K=1,1),I=1,SOLS)
	WRITE (161,485) (CURI(K,X),K=1,1)
	WRITE (161,490) FEVM(X,T),FEVS(X,T),QEV,(QIV(K),K=1,1)
	WRITE (161,500) (SOL(I),FEKM(I,X,T),FEKS(I,X,T),QE(I),
     1  (QI(K,I),K=1,1),I=1,SOLS)
	WRITE (161,505) CURE(X)
C
C
