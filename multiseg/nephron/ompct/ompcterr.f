...
C   MASS CONSERVATION IN THE TIME-DEPENDENT CASE
	PHI(2+KK)=(FVM(X,T)-FVM(X-1,T))/DX
     1  +0.5*SM(X,T)*FEVM(X,T)
     1  +0.5*SM(X-1,T)*FEVM(X-1,T)
     1            +(FVM(X,T-1)-FVM(X-1,T-1))/DX
     1  +0.5*SM(X,T-1)*FEVM(X,T-1)
     1  +0.5*SM(X-1,T-1)*FEVM(X-1,T-1)
     1       +RTAU*(AM(X,T)-AM(X,T-1))
     1       +RTAU*(AM(X-1,T)-AM(X-1,T-1))
	DO 260 I=1,SOLS
	PHI(2+I+KK)=(FKM(I,X,T)-FKM(I,X-1,T))/DX
     1  +0.5*SM(X,T)*FEKM(I,X,T)
     1  +0.5*SM(X-1,T)*FEKM(I,X-1,T)
     1            +(FKM(I,X,T-1)-FKM(I,X-1,T-1))/DX
     1  +0.5*SM(X,T-1)*FEKM(I,X,T-1)
     1  +0.5*SM(X-1,T-1)*FEKM(I,X-1,T-1)
     1   +RTAU*(AM(X,T)*CM(I,X,T)-AM(X,T-1)*CM(I,X,T-1))
     1   +RTAU*(AM(X-1,T)*CM(I,X-1,T)-AM(X-1,T-1)*CM(I,X-1,T-1))
  260   CONTINUE
	DO 262 K=1,2
	PHI(2+KK)=PHI(2+KK)+0.5*
     1   (SM(X,T)*FIVM(K,X,T)+SM(X-1,T)*FIVM(K,X-1,T)+
     1   SM(X,T-1)*FIVM(K,X,T-1)+SM(X-1,T-1)*FIVM(K,X-1,T-1))
	DO 262 I=1,SOLS
	PHI(2+I+KK)=PHI(2+I+KK)+0.5*
     1   (SM(X,T)*FIKM(K,I,X,T)+SM(X-1,T)*FIKM(K,I,X-1,T)+
     1   SM(X,T-1)*FIKM(K,I,X,T-1)+SM(X-1,T-1)*FIKM(K,I,X-1,T-1))
  262   CONTINUE
C
C
...
C
	PHI(3+SOLS+KK)=(FKM(13,X,T)-FKM(13,X-1,T))/DX
     ...
     1 +RTAU*(AM(X,T)*IMPM(X,T)-AM(X,T-1)*IMPM(X,T-1))
     1 +RTAU*(AM(X-1,T)*IMPM(X-1,T)-AM(X-1,T-1)*IMPM(X-1,T-1))
C
	GO TO 290
C
C   MASS CONSERVATION IN THE STEADY-STATE CASE
...
C
	...
	PHI(2+6+KK)=PHI(2+6+KK)+
     1   0.5D6*AM(X,T)*(KHY(5)*CM(6,X,T)-KDHY(5)*CM(5,X,T))+
     1   0.5D6*AM(X-1,T)*(KHY(5)*CM(6,X-1,T)-KDHY(5)*CM(5,X-1,T))+
     1   0.5D6*AM(X,T-1)*(KHY(5)*CM(6,X,T-1)-KDHY(5)*CM(5,X,T-1))+
     1   0.5D6*AM(X-1,T-1)*(KHY(5)*CM(6,X-1,T-1)-KDHY(5)*CM(5,X-1,T-1))
	ELSE
	PHI(2+6+KK)=PHI(2+6+KK)+
     1   0.5D6*AM(X,T)*(KHY(5)*CM(6,X,T)-KDHY(5)*CM(5,X,T))+
     1   0.5D6*AM(X-1,T)*(KHY(5)*CM(6,X-1,T)-KDHY(5)*CM(5,X-1,T))
...
C
	PHI(4+SOLS+KK)=(PM(X,T)-PM(X-1,T))/DX
     1  +0.5*25.17*ETA*MUM*FVM(X,T)/(AM(X,T)**2)
     1  +0.5*25.17*ETA*MUM*FVM(X-1,T)/(AM(X-1,T)**2)
C
	RETURN
	END
