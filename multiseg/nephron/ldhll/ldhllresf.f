...
C
	DOUBLE PRECISION 
     1    PIKM(3,12,101),PJK(3,12,101),PIKS(3,12,101),
     1    CIKM(3,12,101),CJK(3,12,101),CIKS(3,12,101),
     1    GIKM(3,12,101),GJK(3,12,101),GIKS(3,12,101)
C
...
C
	CHARACTER*5 SOL(12)
C
	DOUBLE PRECISION FLUXV(101),FLUXS(12,101),SCALE
C
...
C
	DOUBLE PRECISION JNAHS(101),JCLB(101),JNAP(101),JNAB(101)
C
C
...
C
  ...
  300   FORMAT(//,1X,'DIST',2X,'Na:',3X,'FLUX',6X,'FEKM',6X,'FIKM')
	...
	DO 310 KX=X-CHOP,X
	DIST=DIST+DX
  310   WRITE (107,320) DIST,FLUXS(1,KX),FEKM(1,KX,T),FIKM(1,1,KX,T)
...
C
  ...
  330   FORMAT(//,1X,'DIST',2X,'K:',5X,'FLUX',6X,'FEKM',6X,'FIKM')
	...
	DO 340 KX=X-CHOP,X
	DIST=DIST+DX
  340   WRITE (107,350) DIST,FLUXS(2,KX),FEKM(2,KX,T),FIKM(1,2,KX,T)
...
C
  ...
  360   FORMAT(//,1X,'DIST',2X,'Cl:',3X,'FLUX',6X,'FEKM',6X,'FIKM')
	...
	DO 370 KX=X-CHOP,X
	DIST=DIST+DX
  370   WRITE (107,380) DIST,FLUXS(3,KX),FEKM(3,KX,T),FIKM(1,3,KX,T)
...
C
  ...
  390   FORMAT(//,1X,'DIST',2X,'NH4:',3X,'FLUX',6X,'FEKM',6X,'FIKM',
	...
	DO 400 KX=X-CHOP,X
	DIST=DIST+DX
  400   WRITE (107,410) DIST,FLUXS(11,KX),FEKM(11,KX,T),FIKM(1,11,KX,T),
     1  FIKM(1,10,X,T)
