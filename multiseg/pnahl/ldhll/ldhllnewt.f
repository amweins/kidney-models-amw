	SUBROUTINE LDHLLNEWT(COUNT,INEPH,SWOUT)
C
...
C
	...
	NN=1
	REWIND 104
	READ (104,24) (GAMMA(J),J=1,NN)
   24   FORMAT (D30.20)
	CALL LDHLLGAM(1,GAMMA)
...
C
   49   CALL LDHLLPSET(TAU,TLIM)
	READ(102,50) DUMDT,DUMFVM,
...
C
	...
	IF (TAU.EQ.1) CALL LDHLLPTIM(NPAR)
C
C NOW THAT THE PERITUBULAR PROFILE IS COMPUTED,
...
C IF THE SPATIAL ITERATION FAILS TO CONVERGE THE PROGRAM STOPS.
	PRINT 980
  980   FORMAT (' TOO MANY ITERATIONS IN LDHLL')
	GO TO 600
C
...
C
	IF (TAU.EQ.1) CALL LDHLLRESET(2)
C
	X=X-CHOP
	CALL LDHLLGAM(2,GAMMA)
	X=X+CHOP
	CLOSE (104,STATUS='KEEP')
	OPEN (104,FILE='ldhll/ldhllguess.new')
	WRITE (104,624) (GAMMA(J),J=1,NN)
  624   FORMAT(D30.20)
C
  600   RETURN
	END
