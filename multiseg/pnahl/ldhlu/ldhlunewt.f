	SUBROUTINE LDHLUNEWT(COUNT,SWOUT)
C
...
C
	...
	NN=1
	REWIND 114
	READ (114,24) (GAMMA(J),J=1,NN)
   24   FORMAT (D30.20)
	CALL LDHLUGAM(1,GAMMA)
...
C
   49   CALL LDHLUPSET(TAU,TLIM)
	READ(112,50) DUMDT,DUMFVM,
...
C
	...
	IF (TAU.EQ.1) CALL LDHLUPTIM(NPAR)
C
...
C IF THE SPATIAL ITERATION FAILS TO CONVERGE THE PROGRAM STOPS.
	PRINT 980
  980   FORMAT (' TOO MANY ITERATIONS IN LDHLU')
	GO TO 600
C
...
C
	IF (TAU.EQ.1) CALL LDHLURESET(2)
C
	X=X-CHOP
	CALL LDHLUGAM(2,GAMMA)
	X=X+CHOP
	CLOSE (114,STATUS='KEEP')
	OPEN (114,FILE='ldhlu/ldhluguess.new')
	WRITE (114,624) (GAMMA(J),J=1,NN)
  624   FORMAT(D30.20)
C
  600   RETURN
	END
