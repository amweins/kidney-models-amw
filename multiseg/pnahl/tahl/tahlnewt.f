	SUBROUTINE TAHLNEWT(COUNT,INEPH,SWOUT)
C
...
C
	...
	NN=1
	REWIND 94
	READ (94,24) (GAMMA(J),J=1,NN)
   24   FORMAT (D30.20)
	CALL TAHLGAM(1,GAMMA)
...
C
   49   CALL TAHLPSET(TAU,TLIM)
C  USE INEPH IDENTITY TO RECALIBRATE CHOP AND TL (NO CHANGE IN DX):
	CHOP = (INEPH-1)*(CHOP/5)
	TL = FLOAT(INEPH-1)*TL/5.
C
        GO TO 60
	READ(92,50) DUMDT,DUMFVM,
...
C RESET THE INITIAL PERITUBULAR VALUES FOR THE SHORTER LOOPS
        VS(X,T) = VS(X,T) +
     1   (1.-FLOAT(INEPH-1)/5.)*(VS(CHOP+X,T)-VS(X,T))
        PS(X,T) = PS(X,T) +
     1   (1.-FLOAT(INEPH-1)/5.)*(PS(CHOP+X,T)-PS(X,T))
        IMPS(X,T) = IMPS(X,T) +
     1   (1.-FLOAT(INEPH-1)/5.)*(IMPS(CHOP+X,T)-IMPS(X,T))
        DO 51 I=1,SOLS-1
   51   CS(I,X,T) = CS(I,X,T) +
     1   (1.-FLOAT(INEPH-1)/5.)*(CS(I,CHOP+X,T)-CS(I,X,T))
C
C USE INEPH IDENTITY TO RECALIBRATE CHOP AND TL (NO CHANGE IN DX):
	CHOP = (INEPH-1)*(CHOP/5)
	TL = FLOAT(INEPH-1)*TL/5.
...
C
	...
	IF (TAU.EQ.1) CALL TAHLPTIM(NPAR)
C
...
C IF THE SPATIAL ITERATION FAILS TO CONVERGE THE PROGRAM STOPS.
	PRINT 980
  980   FORMAT (' TOO MANY ITERATIONS IN tAHL')
	GO TO 600
C
...
C
	PRINT 560
  560   FORMAT (' PROBLEM SOLVED FOR THE tAHL')
C
C  FOR THE TRANSIENT EXPERIMENT,THE VARIABLES AT T=1 ARE SET;
C  IF NEEDED, THE GUESS FOR T=2 IS GENERATED.
C
	IF (TAU.EQ.1) CALL TAHLRESET(2)
C
	X=X-CHOP
	CALL TAHLGAM(2,GAMMA)
	X=X+CHOP
	CLOSE (94,STATUS='KEEP')
	OPEN (94,FILE='tahl/tahlguess.new')
	WRITE (94,624) (GAMMA(J),J=1,NN)
  624   FORMAT(D30.20)
C
  600   RETURN
	END
