...
C
C
	WRITE (131,396) TIME,DIST,AM(X,T),SM(X,T)
  396   FORMAT (1H1,/,5X,'TIME=',F15.4,7X,'DIST=',F8.6,
     1  7X,'AM=',D10.4,7X,'SM=',D10.4)
C
C
	WRITE (131,398) L(X,T),CHVL(X,T),(CLVL(K,X,T),K=1,1),AE(X,T)
	WRITE (131,400) VM(X,T),VE(X,T),(VI(K,X,T),K=1,1),VS(X,T)
	WRITE (131,402) PM(X,T),PE(X,T),(PI(K,X,T),K=1,1),PS(X,T)
C
	WRITE (131,410) (SOL(I),
	...
	WRITE (131,420) IMPM(X,T),(IMP(K,X),K=1,1),IMPS(X,T)
	WRITE (131,430) OSMM,OSME,(OSMI(K),K=1,1),OSMS
	WRITE (131,440) LCHM(X),LCHE(X),(LCHI(K,X),K=1,1),LCHS(X)
	WRITE (131,442) (CBUF(K,X,T),K=1,1),(HCBUF(K,X,T),K=1,1)
C
	WRITE (131,460) (SOL(I),XM(I,X)-XS(I,X),XE(I,X)-XS(I,X),
     1      (XI(K,I,X)-XS(I,X),K=1,1),I=1,SOLS)
	WRITE (131,470) (FIVM(K,X,T),JV(K,X,T),FIVS(K,X,T),K=1,1)
	WRITE (131,480) (SOL(I),
     1  (FIKM(K,I,X,T),JK(K,I,X,T),FIKS(K,I,X,T),K=1,1),I=1,SOLS)
	WRITE (131,485) (CURI(K,X),K=1,1)
	WRITE (131,490) FEVM(X,T),FEVS(X,T),QEV,(QIV(K),K=1,1)
	WRITE (131,500) (SOL(I),FEKM(I,X,T),FEKS(I,X,T),QE(I),
     1  (QI(K,I),K=1,1),I=1,SOLS)
	WRITE (131,505) CURE(X)
C
C
