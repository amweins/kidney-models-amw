	SUBROUTINE JMPSTNEWT(COUNT,SWOUT)
C
...
C
	...
	NN=2*(2+SOLS)+1
	REWIND 134
	READ (134,24) (GAMMA(J),J=1,NN)
   24   FORMAT (D30.20)
	CALL JMPSTGAM(1,GAMMA)
...
C
   49   CALL JMPSTPSET(TAU,TLIM)
	READ(132,50) DUMDT,DUMFVM,
     1  DUMVM,VS(X,T),VS(CHOP+X,T),
     1  DUMPM,PS(X,T),PS(CHOP+X,T),
     1  DUMIMPM,IMPS(X,T),IMPS(CHOP+X,T),
     1  (DUMCM,CS(I,X,T),CS(I,CHOP+X,T),I=1,11),
     1  (DUMCM,CS(I,X,T),CS(I,CHOP+X,T),I=13,15)
...
C
	...
	DO 51 I=1,11
   51   CS(I,J,T)=(1.-RJ)*CS(I,X,T) + RJ*CS(I,CHOP+X,T)
	DO 52 I=13,15
...
C
	...
	IF (TAU.EQ.1) CALL JMPSTPTIM(NPAR)
C
...
C
  ...
  940   GAMMA(J)=GAMMA(J)-DELGAM(J)
        IF (GAMMA(51).LE.0.0) GAMMA(51)=DGAM
C
C
...
C
	PRINT 560
  560   FORMAT (' PROBLEM SOLVED FOR THE JMPST')
C
C  FOR THE TRANSIENT EXPERIMENT,THE VARIABLES AT T=1 ARE SET;
C  IF NEEDED, THE GUESS FOR T=2 IS GENERATED.
C
	IF (TAU.EQ.1) CALL JMPSRESET(2)
C
  599   CONTINUE
	X=X-CHOP
	CALL JMPSTGAM(2,GAMMA)
	X=X+CHOP
	CLOSE (134,STATUS='KEEP')
	OPEN (134,FILE='jmpst/jmpstguess.new')
	WRITE (134,624) (GAMMA(J),J=1,NN)
  624   FORMAT(D30.20)
C
  600   RETURN
	END
