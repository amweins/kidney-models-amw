	SUBROUTINE SFPSTNEWT(COUNT,SWOUT)
C
...
C
	...
	NN=2*(2+SOLS)+1
	REWIND 154
	READ (154,24) (GAMMA(J),J=1,NN)
   24   FORMAT (D30.20)
	CALL SFPSTGAM(1,GAMMA)
...
C
   49   CALL SFPSTPSET(TAU,TLIM)
	READ(152,50) DUMDT,DUMFVM,
...
C
	...
	IF (TAU.EQ.1) CALL SFPSTPTIM(NPAR)
C
...
C
	PRINT 560
  560   FORMAT (' PROBLEM SOLVED FOR THE SFPST')
C
C  FOR THE TRANSIENT EXPERIMENT,THE VARIABLES AT T=1 ARE SET;
C  IF NEEDED, THE GUESS FOR T=2 IS GENERATED.
C
	IF (TAU.EQ.1) CALL SFPSRESET(2)
C
  599   CONTINUE
	X=X-CHOP
	CALL SFPSTGAM(2,GAMMA)
	X=X+CHOP
	CLOSE (154,STATUS='KEEP')
	OPEN (154,FILE='sfpst/sfpstguess.new')
	WRITE (154,624) (GAMMA(J),J=1,NN)
  624   FORMAT(D30.20)
C
  600   RETURN
	END
