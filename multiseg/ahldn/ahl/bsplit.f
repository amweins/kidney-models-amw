	PROGRAM BSPLIT
C
C PROGRAM TO GENERATE BOUND.DAT FILES FOR BOTH AHLM AND AHLC THAT
C   PRESERVES THE LINEARITY OF THE BLOOD CONCENTRATIONS
C THE INTERMEDIATE CONCENTRATIONS ARE GIVEN AS CS(I,3)
C   (CS(I,1) AND CS(I,2) ARE THE START AND ENDPOINTS.
C THE PROPORTION IS 0 < ALPHA < 1
...
C
	...
	OPEN (21,FILE='bound1.dat')
	OPEN (22,FILE='bound2.dat')
C
	PRINT 5
    5   FORMAT('FRACTIONAL DISTANCE ALONG SEGMENT 1: ',$)
	READ *, ALPHA
C
	SOLS=12
C
...
C
	VS(3) = (1.D0 - ALPHA)*VS(1) + ALPHA*VS(2)
	PS(3) = (1.D0 - ALPHA)*PS(1) + ALPHA*PS(2)
	IMPS(3) = (1.D0 - ALPHA)*IMPS(1) + ALPHA*IMPS(2)
	DO 52 I=1,SOLS
   52   CS(I,3) = (1.D0 - ALPHA)*CS(I,1) + ALPHA*CS(I,2)
C
	VMAT(1,1)=FVM
	VMAT(2,1)=VM
	VMAT(3,1)=PM
...
C
	WRITE (21,150) DT,FVM,
     1  VM,VS(1),VS(3),PM,PS(1),PS(3),IMPM,IMPS(1),IMPS(3),
     1  (CM(I),CS(I,1),CS(I,3),I=1,SOLS-1)
	WRITE (22,150) DT,FVM,
     1  VM,VS(3),VS(2),PM,PS(3),PS(2),IMPM,IMPS(3),IMPS(2),
     1  (CM(I),CS(I,3),CS(I,2),I=1,SOLS-1)
