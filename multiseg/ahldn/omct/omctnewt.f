...
C
	...
	IERR=0
C
C START THE OMCT CALCULATION
C
	X=X+1
	IF (T.GT.1) GO TO 1049
	NN=2*(2+SOLS)+1
	REWIND 34
	READ (34,1024) (GAMMA(J),J=1,NN)
 1024   FORMAT (D30.20)
	CALL OMCTGAM(1,GAMMA)
C
...
C
 1049   CALL OMCTPSET(TAU,TLIM,CHOP)
	READ(32,1050) DUMDT,DUMFVM,
...
C
	...
	IF (TAU.EQ.1) CALL OMCTPTIM(NPAR)
C
...
C IF THE SPATIAL ITERATION FAILS TO CONVERGE THE PROGRAM STOPS.
	PRINT 1980
 1980   FORMAT (' TOO MANY ITERATIONS IN OMCT')
	WRITE (60,1982) COUNT
 1982   FORMAT ('OMCT ERROR IN EXP. ', I3)
	GO TO 9600
C
C THE SPATIAL STEP CONVERGED,AND
C THE PROGRAM PICKS UP AT THIS POINT:
C
 1558   CALL OMCRESET(1,CHOP)
	CALL OMCTPICK(CHOP)
 1400   CONTINUE
C
...
C
	...
	DIST=DIST + TL*0.5
	X=X+(CHOP/2)
	IF(LL.EQ.1) CALL OMCTRESA
	CALL OMCTRESB
 1410   CALL OMCTRESC
 1420   CALL OMCTRESD(CHOP)
C
	PRINT 1560
 1560   FORMAT (' PROBLEM SOLVED FOR THE OMCT')
C
C  FOR THE TRANSIENT EXPERIMENT,THE VARIABLES AT T=1 ARE SET;
C  IF NEEDED, THE GUESS FOR T=2 IS GENERATED.
C
	IF (TAU.EQ.1) CALL OMCRESET(2,CHOP)
C
	X=X-CHOP
	CALL OMCTGAM(2,GAMMA)
	X=X+CHOP
	CLOSE (34,STATUS='KEEP')
	OPEN (34,FILE='omct/omguess.new')
	WRITE (34,1624) (GAMMA(J),J=1,NN)
 1624   FORMAT(D30.20)
C
 9600   RETURN
	END
