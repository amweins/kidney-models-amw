...
C
C
	WRITE (61,396) TIME,DIST,AM(X,T),SM(X,T)
  396   FORMAT (1H1,/,5X,'TIME=',F15.4,7X,'DIST=',F8.6,
     1  7X,'AM=',D10.4,7X,'SM=',D10.4)
C
C
	WRITE (61,398) L(X,T),CHVL(X,T),(CLVL(K,X,T),K=1,1),AE(X,T)
	WRITE (61,400) VM(X,T),VE(X,T),(VI(K,X,T),K=1,1),VS(X,T)
	WRITE (61,402) PM(X,T),PE(X,T),(PI(K,X,T),K=1,1),PS(X,T)
C
	WRITE (61,410) (SOL(I),
	...
	WRITE (61,412) SOL(SOLS),
     1  CM(SOLS,X,T),CE(SOLS,X,T),(CI(K,SOLS,X,T),K=1,1),CS(SOLS,X,T)
	WRITE (61,420) IMPM(X,T),(IMP(K,X),K=1,1),IMPS(X,T)
	WRITE (61,430) OSMM,OSME,(OSMI(K),K=1,1),OSMS
	WRITE (61,440) LCHM(X),LCHE(X),(LCHI(K,X),K=1,1),LCHS(X)
	WRITE (61,442) (CBUF(K,X,T),K=1,1),(HCBUF(K,X,T),K=1,1)
C
	WRITE (61,460) (SOL(I),XM(I,X)-XS(I,X),XE(I,X)-XS(I,X),
     1      (XI(K,I,X)-XS(I,X),K=1,1),I=1,SOLS)
	WRITE (61,470) (FIVM(K,X,T),JV(K,X,T),FIVS(K,X,T),K=1,1)
	WRITE (61,480) (SOL(I),
     1  (FIKM(K,I,X,T),JK(K,I,X,T),FIKS(K,I,X,T),K=1,1),I=1,SOLS)
	WRITE (61,485) (CURI(K,X),K=1,1)
	WRITE (61,490) FEVM(X,T),FEVS(X,T),QEV,(QIV(K),K=1,1)
	WRITE (61,500) (SOL(I),FEKM(I,X,T),FEKS(I,X,T),QE(I),
...
C
  ...
  500   FORMAT(//,15X,'FEKM',8X,'FEKS',20X,'QE',10X,
     1  'QIP',/,(4X,A5,2D12.4,12X,<1+NCELLS>D12.4))
...
C     1  'CIB',10X,'CS',/,(4X,A5,<3+NCELLS>F12.6))
C  412   FORMAT(4X,A5,1X,<3+NCELLS>D12.4)
C  414   FORMAT(4X,A5,<3+NCELLS>F12.6)
C  420   FORMAT(5X,'IMP',1X,F12.6,12X,4F12.6)
C  430   FORMAT(5X,'OSM',1X,6F12.6)
C  440   FORMAT(6X,'PH',1X,6F12.6)
C  442   FORMAT(4X,'CBUF',1X,24X,<NCELLS>F12.6,
C     1    /,4X,'HCBUF',24X,<NCELLS>F12.6)
C
C  460   FORMAT(//,15X,'XM-XS',7X,'XE-XS',6X,'XIP-XS',
C     1  6X,'XIA-XS',6X,'XIB-XS',/,(4X,A5,<2+NCELLS>F12.4,))
...
C     1  'FIKMB',7X,'JKB',9X,'FIKSB',7X,/,(4X,A5,<3*NCELLS>D12.4))
...
C     1  'QIP',9X,'QIA',9X,'QIB',/,(4X,A5,2D12.4,12X,<1+NCELLS>D12.4))
