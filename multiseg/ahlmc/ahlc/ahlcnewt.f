...
C
	...
	NN=2*(2+SOLS)+1
	REWIND 74
	READ (74,1024) (GAMMA(J),J=1,NN)
 1024   FORMAT (D30.20)
	CALL AHLCGAM(1,GAMMA)
C
...
C
 1049   CALL AHLCPSET(TAU,TLIM,CHOP,ISOFM)
	READ(72,1050) DUMDT,DUMFVM,
...
C
	...
	IF (TAU.EQ.1) CALL AHLCPTIM(NPAR)
C
...
C
 1558   CALL AHLCRESET(1,CHOP)
C	CALL AHLCPICK(CHOP)
 1400   CONTINUE
C
...
C
	...
	CALL AHLCRESB
 1410   CALL AHLCRESC
	WRITE (200,416) 6.D7*FVM(1,1),1.D3*CM(3,X,1),1.D3*CI(1,3,X,1)
  416   FORMAT(8D16.8)
 1420   CALL AHLCRESD(CHOP)
	CALL AHLCRESF(CHOP)
C
	PRINT 1560
 1560   FORMAT (' PROBLEM SOLVED FOR THE AHLC')
C
C  FOR THE TRANSIENT EXPERIMENT,THE VARIABLES AT T=1 ARE SET;
C  IF NEEDED, THE GUESS FOR T=2 IS GENERATED.
C
	IF (TAU.EQ.1) CALL AHLCRESET(2,CHOP)
C
	X=X-CHOP
	CALL AHLCGAM(2,GAMMA)
	X=X+CHOP
	CLOSE (74,STATUS='KEEP')
	OPEN (74,FILE='ahlc/ahlcguess.new')
	WRITE (74,1624) (GAMMA(J),J=1,NN)
 1624   FORMAT(D30.20)
C
 9600   RETURN
	END
