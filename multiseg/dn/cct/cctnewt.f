...
C
	...
	IERR=0
C
C START THE CCT CALCULATION
C
	X=X+1
	IF (T.GT.1) GO TO 1049
	NN=4*(2+SOLS)+1
	REWIND 44
	READ (44,1024) (GAMMA(J),J=1,NN)
 1024   FORMAT (D30.20)
	CALL CCTGAM(1,GAMMA)
C
...
C
 1049   CALL CCTPSET(TAU,TLIM,CHOP)
	READ(42,1050) DUMDT,DUMFVM,
...
C IF THE SPATIAL ITERATION FAILS TO CONVERGE THE PROGRAM STOPS.
	PRINT 1980
 1980   FORMAT (' TOO MANY ITERATIONS IN CCT')
	WRITE (19,1982) COUNT
 1982   FORMAT ('CCT ERROR IN EXP. ', I3)
	GO TO 9600
C
C THE SPATIAL STEP CONVERGED,AND
C THE PROGRAM PICKS UP AT THIS POINT:
C
 1558   CALL CCRESET(1,CHOP)
	CALL CCTPICK(CHOP)
 1400   CONTINUE
C
...
C
	...
	CALL CCTRESB
 1410   CALL CCTRESC
 1420   CALL CCTRESD(CHOP)
C
	PRINT 1560
 1560   FORMAT (' PROBLEM SOLVED FOR THE CCT')
C
C  FOR THE TRANSIENT EXPERIMENT,THE VARIABLES AT T=1 ARE SET;
C  IF NEEDED, THE GUESS FOR T=2 IS GENERATED.
C
	IF (TAU.EQ.1) CALL CCRESET(2,CHOP)
C
	X=X-CHOP
	CALL CCTGAM(2,GAMMA)
	X=X+CHOP
	CLOSE (44,STATUS='KEEP')
	OPEN (44,FILE='cct/ccguess.new')
	WRITE (44,1624) (GAMMA(J),J=1,NN)
 1624   FORMAT(D30.20)
C
 9600   RETURN
	END
