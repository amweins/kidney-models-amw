...
C
C
	INTEGER NWR,IWR(20),JWR(20),CHOP(5),DCHOP(5)
	REAL WRRES(6,20,50),RDRES(20),WRDL(6,20,50),DL(20),
     1    WREX(6,20,50),EX(20),TTIM,WRTIM(6,50),SCALE(20)
...
C
	WRTIM(6,KXP)= WRTIM(1,KXP)+ WRTIM(2,KXP)+ WRTIM(3,KXP)
     1        +WRTIM(4,KXP) +WRTIM(5,KXP)
	DO 857 J=1,NWR
  857   WRRES(6,J,KXP)= WRRES(1,J,KXP)+ WRRES(2,J,KXP)+ WRRES(3,J,KXP)
...
C
  901   KXP=KXP-1
	DO 910 K=1,KXP
C
C FOR J=1 (HCO3), J=2 (TA), AND J=3 (NH4), COMPUTE NET ACID EXCRETION (J=4)
C
	WRDL(1,4,K)=WRDL(1,2,K)+WRDL(1,3,K)-WRDL(1,1,K)
	DO 903 ISEG=1,6
  903   WRRES(ISEG,4,K)=
     1   WRRES(ISEG,1,K)-WRRES(ISEG,2,K)-WRRES(ISEG,3,K)
	WREX(5,4,K)=WREX(5,2,K)+WREX(5,3,K)-WREX(5,1,K)
C
C PLOTTING FOR BARGRAPH INCLUDING HCO3, TA, NH4, NAE
C
	NBARS=28
	WRITE (2,915) NBARS
	ISEG=0
	WRITE (2,915) ISEG, -WRDL(1,1,K),0
	WRITE (2,915) ISEG, WRDL(1,2,K),1
	WRITE (2,915) ISEG, WRDL(1,3,K),2
	WRITE (2,915) ISEG, WRDL(1,4,K),3
	DO 924 ISEG=1,5
	WRITE (2,915) ISEG, WRRES(ISEG,1,K),0
	WRITE (2,915) ISEG, -WRRES(ISEG,2,K),1
	WRITE (2,915) ISEG, -WRRES(ISEG,3,K),2
	WRITE (2,915) ISEG, WRRES(ISEG,4,K),3
  924   CONTINUE
	WRITE (2,915) ISEG, -WREX(5,1,K),0
	WRITE (2,915) ISEG, WREX(5,2,K),1
	WRITE (2,915) ISEG, WREX(5,3,K),2
	WRITE (2,915) ISEG, WREX(5,4,K),3
  910   CONTINUE
