...
C
	DOUBLE PRECISION OSMM(1601),OSMS(1601),PHDIS,
...
C
  ...
  430   FORMAT(//,2X,'DIST',4X,'TA',7X,'PHM',2X,
	...
	DO 440 KX=X-CHOP,X
	DIST=DIST+DX
  440   WRITE (10,450) DIST,TA(KX),LCHM(KX),(XM(I,KX),I=1,SOLS)
  450   FORMAT (F7.4,14F9.4)
C
	WRITE (10,460) (SOL(I),I=1,SOLS-1),SOL(14)
  460   FORMAT(//,2X,'DIST',4X,'OSMM',4X,'FVM',3X,'FKM:',
     1  12(A5,4X))
	DIST=DIST-TL-DX
	DO 470 KX=X-CHOP,X
	DIST=DIST+DX
	WRITE (16,482) DIST,OSMM(KX)
  470   WRITE (10,480) DIST,OSMM(KX),
     1  1.D3*FVM(KX,T),(1.D3*FKM(I,KX,T),I=1,SOLS-1),
     1  1.D3*FVM(KX,T)*TA(KX)
  480   FORMAT (F7.4,6F9.6,D9.3,4F9.6,D9.3,2F9.6)
  482   FORMAT (2E16.8)
C
C
	DO 520 KX=X-CHOP,X
