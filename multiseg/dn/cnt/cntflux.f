...
C	BRNO=0.
	BRNO=2.32
	BR=2.D0**(BRNO*DIST/TL)
	SM(X,T)=6.28D0*RM0*MUM/BR
	AM(X,T)=3.14D0*(RM0**2)*MUM/BR
C
C
...
C
	...
	SS(4)=CI(3,4,X,T)
	CALL AE1(VAE1,SS)
	FIKM(3,3,X,T)=FIKM(3,3,X,T)-NAE1(3)*AI0(3)*VAE1
	FIKM(3,4,X,T)=FIKM(3,4,X,T)+NAE1(3)*AI0(3)*VAE1
C
C
C PUMPS
C
...
C
	...
	SS(4)=CM(12,X,T)
	CALL HK(VHK,SS)
	ATMI(3,2,X)=  2.D0*NPHK(3)*VHK
	ATMI(3,12,X)=-2.D0*NPHK(3)*VHK
	FIKM(3,2,X,T)=FIKM(3,2,X,T)+AI0(3)*ATMI(3,2,X)
	FIKM(3,12,X,T)=FIKM(3,12,X,T)+AI0(3)*ATMI(3,12,X)
C
	ATIS(3,12,X)=LHP(3)/
     1   (1.+EXP(XIHP(3)*(XI(3,12,X)-XS(12,X)-XHP(3))))
	JK(3,12,X,T)=JK(3,12,X,T)+AIE(3)*ATIS(3,12,X)
	FIKS(3,12,X,T)=FIKS(3,12,X,T)+AI0(3)*ATIS(3,12,X)
C
C
 	ATMI(2,2,X)=0.D0
 	ATMI(2,12,X)=-LHP(2)/
