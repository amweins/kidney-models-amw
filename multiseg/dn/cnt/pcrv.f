...
C
	DOUBLE PRECISION RHO(20),DRHO(20),RHOMIN(20)
C
	OPEN (19,FILE='cnparam.tem')
	OPEN (20,FILE='cnparam.dat')
...
C
	RHOMIN(1) = 0.5
	DRHO(1) = 0.1
	RHO(1) = RHOMIN(1) - DRHO(1)
	DO 60 L=2,12
	RHOMIN(L) = 1.0
	DRHO(L) = 0.0
	RHO(L) = RHOMIN(L) - DRHO(L)
   60   CONTINUE
C
	DO 200 KSETS=1,26
	RHO(1) = RHO(1) + DRHO(1)
...
C
	...
	WRITE (20,125)
     1  AIE(K),AI0(K),CLVL0(K),IMP0(K),ZIMP(K),
     1  KHY(K),KDHY(K),TBUF(K),PKB(K),
     1  RHO(1)*NP(K),KNH4(K),NPHK(K),
     1  LHP(K),XIHP(K),XHP(K),NAE1(K)
	WRITE (20,126)
     1  LPMI(K),LPIS(K),
     1  (SMI(K,I),SIS(K,I),RHO(I)*HMI(K,I),HIS(K,I),I=1,SOLS)
...
C
	...
	WRITE (20,166)
     1  LPMI(K),LPIS(K),
     1  (SMI(K,I),SIS(K,I),HMI(K,I),HIS(K,I),I=1,SOLS)
  165   FORMAT (5D12.4,/,4D12.4,/,3D12.4,/,4D12.4)
  166   FORMAT (2D12.4,/,(2F6.3, 2D12.4))
C
	WRITE (20,171) NMI(K),NIS(K)
	IF(NMI(K).EQ.0) GO TO 176
	WRITE (20,171) (II(K,LL),JJ(K,LL),PMI(K,LL),LL=1,NMI(K))
  176   IF(NIS(K).EQ.0) GO TO 180
	WRITE (20,171) (II(K,LL),JJ(K,LL),PIS(K,LL),
     1   LL=NMI(K)+1,NMI(K)+NIS(K))
  171   FORMAT (2I5,D12.4)
C
  180   CONTINUE
C
	WRITE (21,190) RHO(1)
