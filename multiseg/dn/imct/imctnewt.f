...
C
 1049   CALL IMCTPSET(TAU,TLIM,CHOP)
	READ(22,1050) DUMDT,DUMFVM,
...
C
	...
	IF (TAU.EQ.1) CALL IMCTPTIM(NPAR)
C
...
C
	...
	CALL IMCTRESB
 1410   CALL IMCTRESC
 1420   CALL IMCTRESD(CHOP)
C
	PRINT 1560
 1560   FORMAT (' PROBLEM SOLVED FOR THE IMCT')
	CALL EQLIB
C
C  FOR THE TRANSIENT EXPERIMENT,THE VARIABLES AT T=1 ARE SET;
C  IF NEEDED, THE GUESS FOR T=2 IS GENERATED.
C
	IF (TAU.EQ.1) CALL IMCRESET(2,CHOP)
C
	X=X-CHOP
	CALL IMCTGAM(2,GAMMA)
	X=X+CHOP
	CLOSE (24,STATUS='KEEP')
	OPEN (24,FILE='imct/imguess.new')
