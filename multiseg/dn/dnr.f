...
C
  901   KXP=KXP-1
	WRITE (2,902) (SEGNAM(I),I=1,5)
  902   FORMAT(///,10X,5(A4,8X),'TOTAL')
	DO 910 K=1,KXP
	WRITE (2,907)
  907   FORMAT(/,2X,'transit time',/)
	WRITE (2,917) (WRTIM(ISEG,K),ISEG=1,6)
	WRITE (2,903) 
  903   FORMAT(/,2X,'absolute delivery',/)
	DO 923 J=1,NWR
  923   WRITE (2,915) VNAME2(J),(WRDL(ISEG,J,K),ISEG=1,5),WREX(5,J,K)
	WRITE (2,904) 
  904   FORMAT(/,2X,'absolute reabsorption',/)
	DO 924 J=1,NWR
  924   WRITE (2,915) VNAME2(J),(WRRES(ISEG,J,K),ISEG=1,5),WRRES(6,J,K)
	WRITE (2,905) 
  905   FORMAT(/,2X,'reabsorption relative to segmental delivery',/)
	DO 925 J=1,NWR
  925   WRITE (2,916) VNAME2(J),
     1     (WRRES(ISEG,J,K)/WRDL(ISEG,J,K),ISEG=1,5),
     1     WRRES(6,J,K)/WRDL(1,J,K)
	WRITE (2,906) 
  906   FORMAT(/,2X,'reabsorption relative to dct delivery',/)
	DO 926 J=1,NWR
  926   WRITE (2,916) VNAME2(J),
     1     (WRRES(ISEG,J,K)/WRDL(1,J,K),ISEG=1,5),
     1     WRRES(6,J,K)/WRDL(1,J,K)
  910   CONTINUE
