...
C
	...
	REAL WRRES(3601,200),TL,RDRES(3601,20),AVG(40)
...
C
	...
	DO 909 J=1,NWR
	I=1+ISEG
	AVG((K-1)*NWR+J)=0.5*WRRES(I,(K-1)*NWR+J+1)
	DO 905 I=2+ISEG,ISEG+CHOP(KSEG)-1
  905   AVG((K-1)*NWR+J)=AVG((K-1)*NWR+J)+WRRES(I,(K-1)*NWR+J+1)
	I=ISET+CHOP(KSEG)
	AVG((K-1)*NWR+J)=AVG((K-1)*NWR+J)+0.5*WRRES(I,(K-1)*NWR+J+1)
	AVG((K-1)*NWR+J)=AVG((K-1)*NWR+J)/FLOAT(CHOP(KSEG))
  909   CONTINUE
  910   CONTINUE
C
	ISEG=ISEG+CHOP(KSEG)
	WRITE (2,915) (AVG(J),J=1,NWR*KXP)
  915   FORMAT (8E16.8)
  920   CONTINUE
C
	STOP
