...
C
   ...
   30   FORMAT (3I5,2D12.4,/,5D12.4,/,4D12.4,/,
...
C
	PRINT 70
   70   FORMAT('Print the luminal and peritubular scaling factors: ',$)
	READ *, SCALMI,SCALIS
C
	DO 80 K=1,1
	LHP(K)=LHP(K)*SCALMI
	NP(K)=NP(K)*SCALIS
	NNHE3(K)=NNHE3(K)*SCALMI
	LPMI(K)=LPMI(K)*SCALMI
	LPIS(K)=LPIS(K)*SCALIS
	DO 72 I=1,SOLS
	HMI(K,I)=HMI(K,I)*SCALMI
	HIS(K,I)=HIS(K,I)*SCALIS
   72   CONTINUE
	DO 74 LL=1,NMI(K)
   74   PMI(K,LL)=PMI(K,LL)*SCALMI
	DO 76 LL=NMI(K)+1,NMI(K)+NIS(K)
   76   PIS(K,LL)=PIS(K,LL)*SCALIS
	QIAMM=QIAMM*SCALMI
C
   80   CONTINUE
C
...
C
	WRITE (20,130) TAU,TLIM,CHOP,EPSI,TL,
     ...
     1  L0,CHVL0,MUV,LPME,LPES,
     1  (SME(I),SES(I),HME(I),HES(I),I=1,SOLS)
  130   FORMAT (3I5,2D12.4,/,5D12.4,/,4D12.4,/,
     1   5D12.4,/,5D12.4,/,(2F6.3, 2D12.4))
C
	DO 150 K=1,1
	WRITE (20,125)
