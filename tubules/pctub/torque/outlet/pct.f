...
C
	...
	DOUBLE PRECISION GAMMA(80),
     1    PM1,PHIPM1,DPM0,DPM1DPM0
C
C       GAMMA-  VARIABLE VECTOR
C	PM1-	DESIRED OUTLET PRESSURE
C	PHIPM1-	ERROR OF COMPUTED OUTLET PRESSURE
C	DPM0-	INCREMENT IN INLET PRESSURE TO COMPUTE JACOBIAN
C	DPMIDPM0-	JACOBIAN
C
C
        COMMON SOLS,T,X,
...
C
C SOLVE THE TUBULE ITERATIVELY TO GET THE FINAL OUTLET PRESSURE
C
	PM1=PM(1,T)
C
...
C
	ITER=0
C	PM(1,T)=PM1 + 0.6*TL*50.2*ETA*MUM*FVM(1,T)/(AM0**2)
	PM(1,T)=PM1 + TL*50.2*ETA*MUM*FVM(1,T)/(AM0**2)
C	PM(1,T)=PM1 + TL*50.2*ETA*MUM*FVM(1,T)/(AM(1,T)**2)
   90   ITER=ITER+1
	CALL PCNEWTON(COUNT,CHOP,NN,GAMMA)
C
C CHECK TO SEE WHETER THE OUTLET PRESSURE IS CORRECT
C
	PHIPM1=PM(CHOP+1,T)-PM1
	PRINT 92, ITER, PM(1,T), PHIPM1
   92   FORMAT(40X,'ITER= ',I3,5X,'PM(0)= ', F7.3,
     1    5X,'PHIPM1= ',D12.4)
	IF (DABS(PHIPM1).LT.1.D3*EPSI) GO TO 401
C
C IF NOT, THEN A NEWTON ITERATION IS PERFORMED:
C
	DPM0=1.D-8
	PM(1,T)=PM(1,T)+DPM0
	CALL PCNEWTON(COUNT,CHOP,NN,GAMMA)
	PM(1,T)=PM(1,T)-DPM0
C
	DPM1DPM0=(PM(CHOP+1,T)-(PHIPM1+PM1))/DPM0
	IF(ITER.LE.3) PM(1,T)=PM(1,T) - 0.5*PHIPM1/DPM1DPM0
	IF(ITER.GE.4) PM(1,T)=PM(1,T) - PHIPM1/DPM1DPM0
	IF (ITER.LT.10) GO TO 90
	PRINT 95
   95   FORMAT('TOO MANY INLET PRESSURE ITERATIONS')
	STOP
C
