...
C
C Just to scale the NHE3 and H-ATPase alone
C	LHP(K)=LHP0(K)*(1.0 + SCALIS*(TQM(X,T)/TQM0 -1.0))
C	NNHE3(K)=NNHE30(K)*(1.0 + SCALIS*(TQM(X,T)/TQM0 -1.0))
C Just to scale the NBC alone
C	QNBC = 1.5E-08
C	LIS(K,1,1)=LIS0(K,1,1) + QNBC*SCALIS*(TQM(X,T)/TQM0 -1.0)
C	LIS(K,1,4)=LIS0(K,1,4) + 3.*QNBC*SCALIS*(TQM(X,T)/TQM0 -1.0)
C	LIS(K,4,1)=LIS0(K,4,1) + 3.*QNBC*SCALIS*(TQM(X,T)/TQM0 -1.0)
C	LIS(K,4,4)=LIS0(K,4,4) + 9.*QNBC*SCALIS*(TQM(X,T)/TQM0 -1.0)
C
  180   CONTINUE
C
	RETURN
