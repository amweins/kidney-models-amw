...
C TO REPRESENT THE INNER MEDULLARY COLLECTING DUCT EPITHELIUM
...
C	AM-	LUMINAL AREA
C	FVM-	LUMINAL VOLUME FLOW RATE (ML/SEC)
...
C
C
	OPEN (40,FILE='param.dat')
	OPEN (41,FILE='result.dat')
	OPEN (42,FILE='bound.dat')
	OPEN (43,FILE='ptpick.dat')
	OPEN (44,FILE='guess.dat')
	OPEN (46,FILE='ptim.dat')
	OPEN (50,FILE='cdlumen.dat')
	OPEN (51,FILE='cdblood.dat')
C	open (52,FILE='phi.dat')
C
...
C
	...
	DO 52 I=1,SOLS-1
   52   CS(I,J,T)=(1.D0-RJ)*CS(I,1,T) + RJ*CS(I,CHOP+1,T)
...
C
  ...
  230   IF (MAXPHI.LT.DABS(PHI(J))) MAXPHI=DABS(PHI(J))
	PRINT 232,MAXPHI
  232   FORMAT ('+',D12.4)
C	write(52,3233) (phi(ii),ii=1,nn)
C 3233   format (8d12.4)
...
C
	...
	IF(X.EQ.1) CALL IMCTRESA
	DIST=TL*FLOAT(X-1)/FLOAT(CHOP)
	CALL IMCTRESB
  410   CALL IMCTRESC
	X=CHOP+1
  420   CALL IMCTRESD(CHOP)
	CALL IMCTPICK(CHOP)
...
C
	...
	OPEN (44,FILE='guess.new')
	WRITE (44,624) (GAMMA(J),J=1,NN)
  624   FORMAT(D30.20)
	STOP
	END
