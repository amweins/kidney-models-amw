...
C
C
	WRITE (41,398) L(X,T),CHVL(X,T),(CLVL(K,X,T),K=1,1),AE(X,T)
	WRITE (41,400) VM(X,T),VE(X,T),(VI(K,X,T),K=1,1),VS(X,T)
	WRITE (41,402) PM(X,T),PE(X,T),(PI(K,X,T),K=1,1),PS(X,T)
C
	WRITE (41,410) (SOL(I),
	...
	WRITE (41,420) IMPM(X,T),(IMP(K,X),K=1,1),IMPS(X,T)
	WRITE (41,430) OSMM,OSME,(OSMI(K),K=1,1),OSMS
	WRITE (41,440) LCHM(X),LCHE(X),(LCHI(K,X),K=1,1),LCHS(X)
	WRITE (41,442) (CBUF(K,X,T),K=1,1),(HCBUF(K,X,T),K=1,1)
C
	WRITE (41,460) (SOL(I),XM(I,X)-XS(I,X),XE(I,X)-XS(I,X),
     1      (XI(K,I,X)-XS(I,X),K=1,1),I=1,SOLS)
	WRITE (41,470) (FIVM(K,X,T),JV(K,X,T),FIVS(K,X,T),K=1,1)
	WRITE (41,480) (SOL(I),
     1  (FIKM(K,I,X,T),JK(K,I,X,T),FIKS(K,I,X,T),K=1,1),I=1,SOLS)
	WRITE (41,485) (CURI(K,X),K=1,1)
	WRITE (41,490) FEVM(X,T),FEVS(X,T),QEV,(QIV(K),K=1,1)
	WRITE (41,500) (SOL(I),FEKM(I,X,T),FEKS(I,X,T),QE(I),
     1  (QI(K,I),K=1,1),I=1,SOLS)
	WRITE (41,505) CURE(X)
C
C
  398   FORMAT(//,14X,'L',8X,'CHVL',7X,'CLVL ',
     1  9X,'AE',/,6X,4D12.4)
  400   FORMAT(//,16X,'VM',10X,'VE',9X,'VI ',
     1  10X,'VS',/,8X,4F12.3)
  402   FORMAT(//,16X,'PM',10X,'PE',9X,'PI ',
     1  10X,'PS',/,8X,4F12.3)
C
  410   FORMAT(//,16X,'CM',10X,'CE',9X,'CI ',
