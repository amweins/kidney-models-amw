...
C
	REAL*8 AHLVS,AHLPS,AHLIMPS,AHLCS(12)
C AHL DISTANT INTERSTITIAL CONDITIONS
C
        COMMON SOLS,T,X,
     1   Z,RT,RTE,F,
     1   EPSI,DT,RTAU,TIME,DIST,
...
C
	...
	OPEN (52,FILE='ahlbnd.dat')
	OPEN (12,FILE='xy.co2')
	OPEN (53,FILE='fluxes.dat')
C
...
C
	...
	IF(X.EQ.1) CALL DHLRESA
	DIST=TL*FLOAT(X-1)/FLOAT(CHOP)
	CALL DHLRESB
  410   CALL DHLRESC
	X=CHOP+1
  420   CALL DHLRESD(CHOP)
	CALL DHLRESF(CHOP)
C
	REWIND 52
	READ(52,450) AHLVS,AHLPS,AHLIMPS,(AHLCS(I),I=1,SOLS-1)
  450   FORMAT (/,16X,F8.4,/,16X,F8.4,/,16X,F8.4,/,(28X,F14.9))
	REWIND 52
	X=CHOP+1
	WRITE (52,50) DT,FVM(X,T),
     1  VM(X,T),VS(X,T),AHLVS,
     1  PM(X,T),PS(X,T),AHLPS,
     1  IMPM(X,T),IMPS(X,T),AHLIMPS,
     1  (CM(I,X,T),CS(I,X,T),AHLCS(I),I=1,SOLS-1)
C
