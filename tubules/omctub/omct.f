...
C TO REPRESENT THE OUTER MEDULLARY COLLECTING DUCT EPITHELIUM
...
C	AM-	LUMINAL AREA
C	FVM-	LUMINAL VOLUME FLOW RATE (ML/S)
...
C
C
	OPEN (30,FILE='param.dat')
	OPEN (31,FILE='result.dat')
	OPEN (32,FILE='bound.dat')
	OPEN (33,FILE='ptpick.dat')
	OPEN (34,FILE='guess.dat')
	OPEN (36,FILE='ptim.dat')
...
C
	...
	IF(X.EQ.1) CALL OMCTRESA
	DIST=TL*FLOAT(X-1)/FLOAT(CHOP)
	CALL OMCTRESB
  410   CALL OMCTRESC
	X=CHOP+1
  420   CALL OMCTRESD(CHOP)
	CALL OMCTPICK(CHOP)
...
C
	...
	OPEN (34,FILE='guess.new')
	WRITE (34,624) (GAMMA(J),J=1,NN)
  624   FORMAT(D30.20)
	STOP
	END
