...
C
C
	WRITE (31,398) L(X,T),CHVL(X,T),(CLVL(K,X,T),K=2,2),AE(X,T)
	WRITE (31,400) VM(X,T),VE(X,T),(VI(K,X,T),K=2,2),VS(X,T)
	WRITE (31,402) PM(X,T),PE(X,T),(PI(K,X,T),K=2,2),PS(X,T)
C
	WRITE (31,410) (SOL(I),
     1  CM(I,X,T),CE(I,X,T),(CI(K,I,X,T),K=2,2),CS(I,X,T),I=1,4)	
	WRITE (31,412) (SOL(I),
     1  CM(I,X,T),CE(I,X,T),(CI(K,I,X,T),K=2,2),CS(I,X,T),I=5,5)	
	WRITE (31,414) (SOL(I),
     1  CM(I,X,T),CE(I,X,T),(CI(K,I,X,T),K=2,2),CS(I,X,T),I=6,SOLS-3)	
	WRITE (31,412) (SOL(I),
     1  CM(I,X,T),CE(I,X,T),(CI(K,I,X,T),K=2,2),
     1   CS(I,X,T),I=SOLS-2,SOLS-2)	
	WRITE (31,414) (SOL(I),
     1  CM(I,X,T),CE(I,X,T),(CI(K,I,X,T),K=2,2),
     1   CS(I,X,T),I=SOLS-1,SOLS-1)	
	WRITE (31,412) SOL(SOLS),
     1  CM(SOLS,X,T),CE(SOLS,X,T),(CI(K,SOLS,X,T),K=2,2),CS(SOLS,X,T)
	WRITE (31,420) IMPM(X,T),(IMP(K,X),K=2,2),IMPS(X,T)
	WRITE (31,430) OSMM,OSME,(OSMI(K),K=2,2),OSMS
	WRITE (31,440) LCHM(X),LCHE(X),(LCHI(K,X),K=2,2),LCHS(X)
	WRITE (31,442) (CBUF(K,X,T),K=2,2),(HCBUF(K,X,T),K=2,2)
C
	WRITE (31,460) (SOL(I),XM(I,X)-XS(I,X),XE(I,X)-XS(I,X),
     1      (XI(K,I,X)-XS(I,X),K=2,2),I=1,SOLS)
	WRITE (31,470) (FIVM(K,X,T),JV(K,X,T),FIVS(K,X,T),K=2,2)
	WRITE (31,480) (SOL(I),
     1  (FIKM(K,I,X,T),JK(K,I,X,T),FIKS(K,I,X,T),K=2,2),I=1,SOLS)
	WRITE (31,485) (CURI(K,X),K=2,2)
	WRITE (31,490) FEVM(X,T),FEVS(X,T),QEV,(QIV(K),K=2,2)
