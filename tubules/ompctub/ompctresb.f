...
C
C
	WRITE (21,398) L(X,T),CHVL(X,T),(CLVL(K,X,T),K=1,2),AE(X,T)
	WRITE (21,400) VM(X,T),VE(X,T),(VI(K,X,T),K=1,2),VS(X,T)
	WRITE (21,402) PM(X,T),PE(X,T),(PI(K,X,T),K=1,2),PS(X,T)
C
	WRITE (21,410) (SOL(I),
	...
	WRITE (21,420) IMPM(X,T),(IMP(K,X),K=1,2),IMPS(X,T)
	WRITE (21,430) OSMM,OSME,(OSMI(K),K=1,2),OSMS
	WRITE (21,440) LCHM(X),LCHE(X),(LCHI(K,X),K=1,2),LCHS(X)
	WRITE (21,442) (CBUF(K,X,T),K=1,2),(HCBUF(K,X,T),K=1,2)
C
	WRITE (21,460) (SOL(I),XM(I,X)-XS(I,X),XE(I,X)-XS(I,X),
     1      (XI(K,I,X)-XS(I,X),K=1,2),I=1,SOLS)
	WRITE (21,470) (FIVM(K,X,T),JV(K,X,T),FIVS(K,X,T),K=1,2)
	WRITE (21,480) (SOL(I),
     1  (FIKM(K,I,X,T),JK(K,I,X,T),FIKS(K,I,X,T),K=1,2),I=1,SOLS)
	WRITE (21,485) (CURI(K,X),K=1,2)
	WRITE (21,490) FEVM(X,T),FEVS(X,T),QEV,(QIV(K),K=1,2)
