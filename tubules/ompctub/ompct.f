...
C THIS MODEL IS USED TO GENERATE AN OMCT MODEL THAT HAS AN ACTIVE
C PRINCIPAL CELL (FROM CCT) COMBINED WITH OMCT ALPHA CELL (12/17/08)
C
...
C     INDICES FOR THE CELL TYPE: 1=PRINCIPAL; 2=ALPHA
C
...
C
	...
	IF(X.EQ.1) CALL OMPCTRESA
	DIST=TL*FLOAT(X-1)/FLOAT(CHOP)
	CALL OMPCTRESB
  410   CALL OMPCTRESC
	X=CHOP+1
  420   CALL OMPCTRESD(CHOP)
	CALL OMPCTPICK(CHOP)
