...
C REVISED (5/21/04) TO INCLUDE CONTROLLED TRANSPORTERS
C
...
C
C	HCON-	VALUE OF CONTROLLED VARIABLES
C 	HCON0-	BASELINE VALUE OF CONTROLLED VARIABLE
C	DHCON-	SENSITIVITY OF CONTROLLED VARIABLE TO ITS CONTROLLERS
...
C
	...
	IF(X.EQ.1) CALL DCTRESA
	DIST=TL*FLOAT(X-1)/FLOAT(CHOP)
	CALL DCTRESB
  410   CALL DCTRESC
	X=CHOP+1
  420   CALL DCTRESD(CHOP)
C
