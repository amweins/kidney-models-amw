...
C
        KNPN(1)=0.0002*(1.D0+CI(1,2,X,T)/.00833)
        KNPK(1)=0.0001*(1.D0+CS(1,X,T)/.0185)
        ATIS(1,1,X)=NP(1)*(CI(1,1,X,T)/(KNPN(1)+CI(1,1,X,T)))**3*
     1    (CS(2,X,T)/(KNPK(1)+CS(2,X,T)))**2
C
	CON(1)=CLVL(1,X,T)
C	CON(2)=CI(1,4,X,T)
	CON(2)=CI(1,3,X,T)
	CON(3)=ATIS(1,1,X)*(AIE(1)+AI0(1))
	CON(4)=CI(1,1,X,T)
C
	DO 80 I=1,7
	HCON(I) = HCON0(I)
