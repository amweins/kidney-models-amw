...
C SCALES UP THE PARAMETERS FOR ALPHA IC LUMINAL AND PERITUBULAR CELL MEMBRANES- UNIFORMLY
C
...
C
	DOUBLE PRECISION SCALMI(3),SCALIS(3),
...
C
	...
	SCALMI(2) =1.0
	SCALMI(3) =1.0
	ELSE IF (KSETS.EQ.2) THEN
	SCALMI(1) =2.0
	ELSE IF (KSETS.EQ.3) THEN
	SCALMI(3) =5.0
	ELSE IF (KSETS.EQ.4) THEN
	SCALMI(2) =4.0
	ENDIF
	SCALIS(1) = SCALMI(1)
	SCALIS(2) = SCALMI(2)
...
C
	DO 80 K=1,3
	LHP(K)=LHP0(K)*SCALMI(K)
	NP(K)=NP0(K)*SCALIS(K)
	NPHK(K)=NPHK0(K)*SCALIS(K)
	LPMI(K)=LPMI0(K)*SCALMI(K)
	LPIS(K)=LPIS0(K)*SCALIS(K)
	DO 72 I=1,SOLS
	HMI(K,I)=HMI0(K,I)*SCALMI(K)
	HIS(K,I)=HIS0(K,I)*SCALIS(K)
   72   CONTINUE
	DO 74 LL=1,NMI(K)
   74   PMI(K,LL)=PMI0(K,LL)*SCALMI(K)
	DO 76 LL=NMI(K)+1,NMI(K)+NIS(K)
   76   PIS(K,LL)=PIS0(K,LL)*SCALIS(K)
C
   80   CONTINUE
C
C
