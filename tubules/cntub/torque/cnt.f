...
C	SWBR-	PARAMETER TO SPECIFY COALESCING OR THROUGH TUBULE
C
...
C
c	PRINT 25
   25   FORMAT(' COALESCING (coal) OR THROUGH (thru) TUBULE: ',$)
c	READ *, SWBR
	swbr = 'thru'
	PRINT 27
...
C
	...
	IF(X.EQ.1) CALL CNTRESA
	DIST=TL*FLOAT(X-1)/FLOAT(CHOP)
	CALL CNTRESB
  410   CALL CNTRESC
	X=CHOP+1
  420   CALL CNTRESD(CHOP)
	CALL CNTORQUE(3,CHOP)
C
