	SUBROUTINE CNTORQUE(SWT,CHOP,SWBR)
C
...
C
	...
	SM(X,T)=6.28D0*RMT0*MUM/BR
	RM(X,T)=RM0*(1.0 + MUR*(PM(X,T)-PS(X,T)))
	AM(X,T)=3.14D0*(RM(X,T)**2)*MUM/BR
	TQM(X,T)=(FVM(X,T)/AM(X,T))
C
	DO 180 K=1,3
	LHP(K)=LHP0(K)*(1.0 + SCALMI(K)*(TQM(X,T)/TQM0 -1.0))
	NP(K)=NP0(K)*(1.0 + SCALIS(K)*(TQM(X,T)/TQM0 -1.0))
	NPHK(K)=NPHK0(K)*(1.0 + SCALMI(K)*(TQM(X,T)/TQM0 -1.0))
	NAE1(K)=NAE10(K)*(1.0 + SCALIS(K)*(TQM(X,T)/TQM0 -1.0))
