...
C
	...
	CO2FLX = (FEKM(6,X,T) + 
     1    FIKM(1,6,X,T)+FIKM(2,6,X,T)+FIKM(3,6,X,T))*SM(X,T)
C
C 
...
C IF THE SPATIAL ITERATION FAILS TO CONVERGE
	PRINT 40
   40   FORMAT (' TOO MANY ITERATIONS IN CNT:EQLIB')
	STOP
C
...
C IF THE SPATIAL ITERATION FAILS TO CONVERGE
	PRINT 140
  140   FORMAT (' TOO MANY ITERATIONS IN CCT:EQLIB')
	STOP
C
C THE SPATIAL STEP CONVERGED,AND
C THE PROGRAM PICKS UP AT THIS POINT:
