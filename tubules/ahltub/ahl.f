...
C
   ...
   27   FORMAT(' NUMBER OF EXPERIMENTS = ',$)
	...
	DO 600 COUNT=1,EXP
C
	CALL AHLPSET(TAU,TLIM,CHOP,ISOFM)
...
C
	...
	IF(X.EQ.1) CALL AHLRESA
	DIST=TL*FLOAT(X-1)/FLOAT(CHOP)
	CALL AHLRESB
  410   CALL AHLRESC
	X=CHOP+1
  420   CALL AHLRESD(CHOP)
	CALL AHLRESF(CHOP)
C
	X=CHOP+1
